...
#ifdef CONFIG_MACH_SDM450_CV7A_KR
    { ES9218P_REG_07,        0x00 },    // Filter Bandwidth and System Mute
#else
...
static int g_sabre_cf_num = 6; // default = 6
static int g_dop_flag = 0;
static int g_auto_mute_flag = 0;
#ifdef ES9218P_DEBUG
static int g_debug_delay = 500; // ESS pop-click debugging step time delay
#endif
#ifdef CONFIG_MACH_SDM450_CV5A_LAO_COM
static u8  normal_harmonic_comp_left[4] = {0x0a, 0x00, 0x1e, 0xfb};
static u8  normal_harmonic_comp_right[4] = {0x0a, 0x00, 0x1e, 0xfb};
static u8  advance_harmonic_comp_left[4] = {0x26, 0x02, 0x9c, 0xff};
static u8  advance_harmonic_comp_right[4] = {0xc2, 0x01, 0x9c, 0xff};
static u8  aux_harmonic_comp_left[4] = {0x2c, 0x01, 0x06, 0xfe};
static u8  aux_harmonic_comp_right[4] = {0xd2, 0x00, 0x06, 0xfe};
#elif defined(CONFIG_MACH_SDM450_MH3J)
static u8  normal_harmonic_comp_left[4] = {0x00, 0x00, 0x40, 0xf3};
static u8  normal_harmonic_comp_right[4] = {0xc0, 0xff, 0x50, 0xf4};
static u8  advance_harmonic_comp_left[4] = {0x30, 0x02, 0xd8, 0xff};
static u8  advance_harmonic_comp_right[4] = {0x00, 0x02, 0xe8, 0xff};
static u8  aux_harmonic_comp_left[4] = {0x40, 0x01, 0xff, 0x00};
static u8  aux_harmonic_comp_right[4] = {0x00, 0x01, 0x20, 0xfe};
#else
static u8  normal_harmonic_comp_left[4] = {0x0a, 0x00, 0x1e, 0xfb};
static u8  normal_harmonic_comp_right[4] = {0x0a, 0x00, 0x1e, 0xfb};
static u8  advance_harmonic_comp_left[4] = {0xc2, 0x01, 0x0a, 0x00};
static u8  advance_harmonic_comp_right[4] = {0x68, 0x01, 0x28, 0x00};
static u8  aux_harmonic_comp_left[4] = {0xd2, 0x00, 0x11, 0xfe};
static u8  aux_harmonic_comp_right[4] = {0xd2, 0x00, 0x2a, 0xfe};
#endif
enum {
...
static void es9218_reset_gpio_H(void)
{
    ...
    if(g_es9218_priv->es9218_data->use_dpdt_elect_ins)
        gpio_set_value(g_es9218_priv->es9218_data->pm_dac_sw_sel, 1);
...
static void es9218_reset_gpio_L(void)
{
    ...
    if(g_es9218_priv->es9218_data->use_dpdt_elect_ins)
        gpio_set_value(g_es9218_priv->es9218_data->pm_dac_sw_sel, 0);
...
static int es9218_normal_harmonic_comp_put_left(struct snd_kcontrol *kcontrol,
    ...
    for( i =0  ;  i < 4  ; i ++ ) {
        ...
        pr_info("%s: normal_harmonic_comp_left[%d] %d", __func__, i , normal_harmonic_comp_left[i]);
    }
    return 0 ;
}
        ...
        struct es9218_data *pdata)
{
    ...
    */
    ...
    /*
     * Value that activates the LDO connected to the switch related on by-pass hifi.
    */

    pdata->enable_ldo_comp = of_get_named_gpio(dev->of_node, "dac,hifi-ldo-comp", 0);
    if (pdata->enable_ldo_comp < 0) {
        pr_err("Looking up %s property in node %s failed %d\n", "dac,hifi-ldo-comp", dev->of_node->full_name, pdata->enable_ldo_comp);
        goto err;
    }

    pr_info("%s: enable hifi-ldo-comp gpio %d\n", __func__, pdata->enable_ldo_comp);

    if( of_property_read_bool(dev->of_node, "dac,pm-dac-sw-sel") &&
            of_property_read_bool(dev->of_node, "dac,gnd-sel") ) {

        pdata->use_dpdt_elect_ins = true;
        pdata->pm_dac_sw_sel = of_get_named_gpio(dev->of_node,
                "dac,pm-dac-sw-sel", 0);
        pdata->gnd_sel = of_get_named_gpio(dev->of_node,
                "dac,gnd-sel", 0);

        if ((pdata->pm_dac_sw_sel < 0) || (pdata->gnd_sel < 0)) {
            pr_err("Looking up %s property in node %s failed %d\n", "dac,pm-dac-sw-sel", dev->of_node->full_name, pdata->pm_dac_sw_sel);
            pr_err("Looking up %s property in node %s failed %d\n", "dac,gnd-sel", dev->of_node->full_name, pdata->gnd_sel);
            goto err;
        }

        pr_info("%s: pm dat switch gpio %d", __func__, pdata->pm_dac_sw_sel);
        pr_info("%s: gnd sel gpio %d", __func__, pdata->gnd_sel);
    }
    else {
        pdata->use_dpdt_elect_ins = false;
    }
...
//  struct snd_soc_codec *codec = dai->codec;
    ...
    if (( es9218_power_state == ESS_PS_IDLE ) && ( es9218_is_amp_on != 0 ) )
        es9218_power_state = ESS_PS_BYPASS;
...
static int es9218_probe(struct i2c_client *client,const struct i2c_device_id *id)
{
    ...
    ret = gpio_request(pdata->enable_ldo_comp, "comp_ldo");
    if (ret < 0) {
        pr_err("%s(): ess reset request failed\n", __func__);
        goto ldo_gpio_request_error;
    }
    ret = gpio_direction_output(pdata->enable_ldo_comp, 1);
    if (ret < 0) {
        pr_err("%s: ldo_comp set failed\n", __func__);
        goto ldo_gpio_request_error;
    }
    gpio_set_value(pdata->enable_ldo_comp, 1);

    if( pdata->use_dpdt_elect_ins ) {
        ret = gpio_request(pdata->pm_dac_sw_sel, "ess_switch");
        if (ret < 0) {
            pr_err("%s(): ess switch request failed\n", __func__);
            gpio_free(pdata->pm_dac_sw_sel);
            return ret;
        }
        ret = gpio_direction_output(pdata->pm_dac_sw_sel, 1);

        ret = gpio_request(pdata->gnd_sel, "gnd_switch");
        if (ret < 0) {
            pr_err("%s(): gnd switch request failed\n", __func__);
            gpio_free(pdata->gnd_sel);
            return ret;
        }
        ret = gpio_direction_output(pdata->gnd_sel, 0);
    }
...
ldo_gpio_request_error:
    gpio_free(pdata->enable_ldo_comp);
#if 0
