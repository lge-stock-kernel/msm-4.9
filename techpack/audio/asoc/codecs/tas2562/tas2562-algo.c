...
#include <dsp/tas2562_amp.h>

//TODO: Move below to tas2562_amp.h header file
/*Re-calibration*/
#ifndef SMARTAMP_SPEAKER_CALIBDATA_FILE
...
int get_calibrated_re_tv(int *Rdc_fix, int *Tv_fix)
{
	struct file *pF = NULL;
	loff_t pos = 0;
	...
	fs = get_fs();
	set_fs(get_ds());
	pr_debug("[Smartamp:%s] file %s open \n", __func__, SMARTAMP_SPEAKER_CALIBDATA_FILE);
	pF = filp_open(SMARTAMP_SPEAKER_CALIBDATA_FILE , O_RDWR, 0666);
	if(IS_ERR(pF))
	{
		pr_debug("[Smartamp:%s] file %s open \n", __func__, SMARTAMP_SPEAKER_CALIBDATA_FILE2);
		pF = filp_open(SMARTAMP_SPEAKER_CALIBDATA_FILE2 , O_RDWR, 0666);
	}
	if(!IS_ERR(pF))
	{
				...
				struct snd_ctl_elem_value *pUcontrol)
{
	...
	spk_id = user_data;
	pr_debug("[Smartamp:%s] Setting Speaker ID %d", __func__, user_data);
	...
	struct snd_ctl_elem_value *pUcontrol)
{
	...
	pr_debug("[Smartamp:%s] Getting Speaker ID %d", __func__, user_data);
				...
				struct snd_ctl_elem_value *pUcontrol)
{
	...
	pr_debug("[Smartamp:%s] Setting profile %s", __func__, profile_index_text[user_data]);
	//To exclude None
	if(user_data)
		user_data -= 1;
	else
	...
	{
		pr_debug("[Smartamp:%s] Sending Rx Config", __func__);
				...
				struct snd_ctl_elem_value *pUcontrol)
{
	...
	pUcontrol->value.integer.value[0] = user_data;
	pr_debug("[Smartamp:%s] getting profile %s", __func__, profile_index_text[user_data]);
	...
	struct snd_ctl_elem_value *pUcontrol)
{
	...
	pr_debug("[Smartamp:%s] case %s", __func__, tas2562_calib_test_text[user_data]);
	...
	struct snd_ctl_elem_value *pUcontrol)
{
	...
	pr_debug("[Smartamp:%s] case %s", __func__, tas2562_calib_test_text[calib_state]);
	return ret;
}
	...
	struct snd_ctl_elem_value *pUcontrol)
{
	...
	pUcontrol->value.integer.value[0] = user_data;
	pr_debug("[Smartamp:%s] Getting Re %d", __func__, user_data);
	...
	struct snd_ctl_elem_value *pUcontrol)
{
	...
	pUcontrol->value.integer.value[0] = user_data;
	pr_debug("[Smartamp:%s] Getting F0 %d", __func__, user_data);
	...
	struct snd_ctl_elem_value *pUcontrol)
{
	...
	pUcontrol->value.integer.value[0] = user_data;
	pr_debug("[Smartamp:%s] Getting Q %d", __func__, user_data);
	...
	struct snd_ctl_elem_value *pUcontrol)
{
	...
	pUcontrol->value.integer.value[0] = user_data;
	pr_debug("[Smartamp:%s] Getting Tv %d", __func__, user_data);
				...
				struct snd_ctl_elem_value *pUcontrol)
{
	...
	pr_debug("[Smartamp:%s] case %s", __func__, tas2562_smartamp_enable_text[user_data]);

	smartamp_enable = smartamp_set_val(user_data);
	if(smartamp_get_val(smartamp_enable) == FALSE)
	{
		pr_debug("[Smartamp:%s] Disable called", __func__);
		calib_state = 0;
		calib_re = 0;
		return 0;
	}
	...
	{
		pr_debug("[Smartamp:%s] Setting Tcal %d", __func__, calib_tv);
		...
		pr_debug("[Smartamp:%s] Setting Re %d", __func__, calib_re);
	...
	pr_debug("[Smartamp:%s] Setting the feedback module info for TAS", __func__);
	...
	pr_debug("[Smartamp:%s] Sending TX Enable", __func__);
	...
	pr_debug("[Smartamp:%s] Sending RX Enable", __func__);
				...
				struct snd_ctl_elem_value *pUcontrol)
{
	...
	pr_debug("[Smartamp:%s] case %s", __func__, tas2562_smartamp_bypass_text[user_data]);
	return ret;
}
	...
	struct snd_ctl_elem_value *pUcontrol)
{
	...
	if(ret < 0)
	{
		...
		pr_debug("[Smartamp:%s] status = %d", __func__, user_data);
	data &= user_data;
	user_data = 0;
	param_id = CAPI_V2_TAS_RX_ENABLE;
	ret = afe_smartamp_algo_ctrl((u8 *)&user_data, param_id,
	...
	if(ret < 0)
	{
		...
		pr_debug("[SmartAmp:%s] Rx Enable = %d", __func__, user_data);
	...
	if(ret < 0)
	{
		...
		pr_debug("[Smartamp:%s] Tx Enable = %d", __func__, user_data);

	data &= user_data;
	pUcontrol->value.integer.value[0] = data;
	pr_debug("[Smartamp:%s] case %s", __func__, tas2562_smartamp_status_text[data]);
