...
static void irq_work_routine(struct work_struct *work)
{
	...
	if (pTAS2562->mbRuntimeSuspend) {
		dev_dbg(pTAS2562->dev, "%s, Runtime Suspended\n", __func__);
		goto end;
	}

	if (pTAS2562->mnPowerState == TAS2562_POWER_SHUTDOWN) {
		dev_dbg(pTAS2562->dev, "%s, device not powered\n", __func__);
		goto end;
	}
	...
	if (((nDevInt1Status & 0x3) != 0) || ((nDevInt2Status & 0x0f) != 0)) {
		...
		while (nCounter > 0) {
			...
			dev_dbg(pTAS2562->dev, "IRQ reg is: %s %d, %d\n", __func__, irqreg, __LINE__);
			...
			dev_dbg(pTAS2562->dev, "set ICN to -80dB\n");
...
reload:
	...
	if (nResult >= 0) {
	    ...
	    dev_dbg(pTAS2562->dev, "enable IRQ\n");
	}

	nResult = gpio_get_value(pTAS2562->mnIRQGPIO);
	dev_dbg(pTAS2562->dev, "%s, irq GPIO state: %d\n", __func__, nResult);
	...
	if (!hrtimer_active(&pTAS2562->mtimer)) {
		...
		dev_dbg(pTAS2562->dev, "timer not active\n");
