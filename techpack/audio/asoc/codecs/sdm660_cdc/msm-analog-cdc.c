					       ...
					       int event, bool enable)
{
	...
	case MBHC_COMMON_MICB_SET_VAL:
		...
		if(lge_get_factory_boot()){
			val = (enable ? 0x50 : 0x00);
			pr_info("%s, [LGE MBHC] msm_anlg_cdc_mbhc_common_micb_ctrl Factory return enable %d, val %x\n", __func__, enable, val);
		}
#endif
		break;
...
#ifdef CONFIG_LGE_USB_FACTORY
	if(lge_get_factory_boot()) {
		pr_info("%s, [LGE MBHC] wcd_imped_config Factory return imped %d\n", __func__, imped);
		return;
	}
#endif
