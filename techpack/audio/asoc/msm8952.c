...
static int mi2s_quat_rx_sample_rate = SAMPLING_RATE_48KHZ;
static int mi2s_quat_rx_bit_format = SNDRV_PCM_FORMAT_S24_LE;
static int mi2s_quat_rx_bits_per_sample = 32;
#endif
...
#if defined(CONFIG_SND_SOC_TAS2560) || defined(CONFIG_SND_SOC_TAS2562)
#define CONFIG_SND_SOC_TAS_SPK_PROT
#endif
#endif /*CONFIG_SND_SOC_QUIN_HIFI*/

#ifdef CONFIG_SND_SOC_TAS_SPK_PROT
static int msm_quin_mi2s_tx_ch = 2;
#endif
...
#define TAS_CODEC_NAME  "tas2560.5-004f"
#define TAS_DAI_NAME    "tas2560 ASI1"
#elif defined(CONFIG_SND_SOC_TAS2562)
#define TAS_CODEC_NAME  "tas2562.2-004c"
#define TAS_DAI_NAME    "tas2562 ASI1"
#endif
...
#if defined(CONFIG_SND_LGE_PD_SWITCH)
static int dp_sw = -1;
static int dp_wifi = 0;
#endif
...
#if defined(CONFIG_SND_SOC_USE_QUAT_MI2S)
...
#define CONFIG_SND_RENAME_CARD
#define CONFIG_SND_CHECK_EXT_AMP
#endif
#if defined(CONFIG_MACH_MSM8917_MH5LM) || defined(CONFIG_MACH_MSM8917_MH5LM_NONAB)
#include <soc/qcom/lge/board_lge.h>
extern enum lge_sku_carrier_type lge_get_sku_carrier(void);
#endif

#if defined(CONFIG_SND_RENAME_CARD)
static char init_card_name[32] = DRV_NAME;
#endif
#if defined(CONFIG_SND_CHECK_EXT_AMP)
static int lge_get_smart_pa(void)
{
	int sku;
	int support;
#if defined(CONFIG_MACH_MSM8917_MH5LM) || defined(CONFIG_MACH_MSM8917_MH5LM_NONAB)
	sku = lge_get_sku_carrier();
	support = (sku == TMUS)? 1: 0;
	printk(KERN_INFO "smart pa support = %d(%d)\n",support,sku);
#else
	sku = 0;
	support = 0;
#endif
...
int msm_init_hifi_gpio(struct platform_device *pdev, struct msm_asoc_mach_data *pdata)
{
	if(!pdev || !pdata)
		return 0;
...
#ifdef CONFIG_SND_SOC_HAC_PA
int msm_init_hac_pa_gpio(struct platform_device *pdev, struct msm_asoc_mach_data *pdata)
{
	if(!pdev || !pdata)
		return 0;
				...
				struct snd_pcm_hw_params *params)
{
	...
	param_set_mask(params, SNDRV_PCM_HW_PARAM_FORMAT,
		mi2s_quat_rx_bit_format);
	rate->min = rate->max = mi2s_quat_rx_sample_rate;
...
static uint32_t get_mi2s_rx_clk_val(int port_id)
{
	...
	if(port_id==AFE_PORT_ID_QUATERNARY_MI2S_RX){
		clk_val = (mi2s_quat_rx_sample_rate * mi2s_quat_rx_bits_per_sample * 2);
		pr_debug("%s: Quaternary MI2S Rx bit clock value: 0x%0x\n", __func__, clk_val);
		return clk_val;
	}
#endif
#ifdef CONFIG_SND_SOC_QUIN_HIFI
...
#if defined(CONFIG_SND_SOC_USE_QUAT_MI2S)
static int msm_quat_mi2s_sclk_ctl(struct snd_pcm_substream *substream, bool enable)
{
	int ret = 0;
	...
	if (port_id < 0 || port_id != AFE_PORT_ID_QUATERNARY_MI2S_RX) {
		return -EINVAL;
	}
	...
	pr_debug("%s: Quat MI2S clk ctl enterd\n", __func__);

	if (enable) {
		pr_info("%s: afe_clk_ver v2=%d\n", __func__,pdata->afe_clk_ver);
		quat_mi2s_rx_clk.enable = enable;
		quat_mi2s_rx_clk.clk_id = msm8952_get_clk_id(port_id);
		quat_mi2s_rx_clk.clk_freq_in_hz =
				get_mi2s_rx_clk_val(port_id);
		ret = afe_set_lpass_clock_v2(port_id,&quat_mi2s_rx_clk);
		if (ret < 0)
...
#if defined(CONFIG_SND_SOC_USE_QUAT_MI2S)
	if(!msm_quat_mi2s_sclk_ctl(substream,enable)){
		return 0;
	}
#endif
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	switch (mi2s_quat_rx_bit_format) {
	case SNDRV_PCM_FORMAT_S24_3LE:
		ucontrol->value.integer.value[0] = 2;
		break;
	...
	pr_debug("%s: mi2s_quat_rx_bit_format = %d, ucontrol value = %ld\n",
			__func__, mi2s_quat_rx_bit_format,
			ucontrol->value.integer.value[0]);
	...
	struct snd_ctl_elem_value *ucontrol)
{
	switch (ucontrol->value.integer.value[0]) {
	case 2:
		mi2s_quat_rx_bit_format = SNDRV_PCM_FORMAT_S24_3LE;
		mi2s_quat_rx_bits_per_sample = 32;
		break;
	case 1:
		mi2s_quat_rx_bit_format = SNDRV_PCM_FORMAT_S24_LE;
		mi2s_quat_rx_bits_per_sample = 32;
		break;
	case 0:
	default:
		mi2s_quat_rx_bit_format = SNDRV_PCM_FORMAT_S16_LE;
		mi2s_quat_rx_bits_per_sample = 16;
		break;
	}
	return 0;
}
...
static int msm_quat_mi2s_rx_sample_rate_get(struct snd_kcontrol *kcontrol,
				struct snd_ctl_elem_value *ucontrol)
{
	int sample_rate_val = 0;

	switch (mi2s_quat_rx_sample_rate) {
	case SAMPLING_RATE_192KHZ:
		sample_rate_val = 6;
		break;
	case SAMPLING_RATE_96KHZ:
...
static int msm_quat_mi2s_rx_sample_rate_put(struct snd_kcontrol *kcontrol,
				    struct snd_ctl_elem_value *ucontrol)
{
	switch (ucontrol->value.integer.value[0]) {
	case 0:
		mi2s_quat_rx_sample_rate = SAMPLING_RATE_8KHZ;
		break;
	case 1:
		mi2s_quat_rx_sample_rate = SAMPLING_RATE_16KHZ;
		break;
	case 2:
		mi2s_quat_rx_sample_rate = SAMPLING_RATE_32KHZ;
		break;
	case 3:
		mi2s_quat_rx_sample_rate = SAMPLING_RATE_44P1KHZ;
		break;
	case 5:
		mi2s_quat_rx_sample_rate = SAMPLING_RATE_96KHZ;
		break;
	case 6:
		mi2s_quat_rx_sample_rate = SAMPLING_RATE_192KHZ;
		break;
	case 4:
	default:
		mi2s_quat_rx_sample_rate = SAMPLING_RATE_48KHZ;
		break;
	}
	pr_debug("%s: mi2s_rx_sample_rate = %d\n", __func__,
		 mi2s_quat_rx_sample_rate);
	return 0;
}
#endif /*CONFIG_SND_SOC_USE_QUAT_MI2S*/
...
#endif /*CONFIG_SND_SOC_QUIN_HIFI*/
#if defined(CONFIG_SND_SOC_TAS_SPK_PROT)
static int msm_quin_mi2s_tx_ch_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s: msm_quin_mi2s_tx_ch  = %d\n", __func__,
		 msm_quin_mi2s_tx_ch);
	ucontrol->value.integer.value[0] = msm_quin_mi2s_tx_ch - 1;
	return 0;
}

static int msm_quin_mi2s_tx_ch_put(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
	msm_quin_mi2s_tx_ch = ucontrol->value.integer.value[0] + 1;
	pr_debug("%s: msm_quin_mi2s_tx_ch = %d\n", __func__, msm_quin_mi2s_tx_ch);
	return 1;
}
...
#ifdef CONFIG_MACH_LGE
static const struct soc_enum custom_snd_enum[] = {
...
static const struct snd_kcontrol_new custom_snd_controls[] = {
#ifdef CONFIG_SND_SOC_HIFI_CTRL
	SOC_ENUM_EXT("HiFi Function", custom_snd_enum[0], msm_hifi_get,
			msm_hifi_put),
#endif
#ifdef CONFIG_SND_SOC_HAC_PA
	SOC_ENUM_EXT("HAC_PA_AW8155_MODE", custom_snd_enum[1],
			hac_pa_aw8155_mode_get, hac_pa_aw8155_mode_put),
#endif
#if defined(CONFIG_SND_SOC_USE_QUAT_MI2S)
	SOC_ENUM_EXT("QUAT_MI2S_RX BitWidth", msm_snd_enum[0],
			msm_quat_mi2s_rx_bit_format_get, msm_quat_mi2s_rx_bit_format_put),
	SOC_ENUM_EXT("QUAT_MI2S_RX Channels", msm_snd_enum[1],
			msm_quat_mi2s_rx_ch_get, msm_quat_mi2s_rx_ch_put),
	SOC_ENUM_EXT("QUAT_MI2S_RX SampleRate", custom_snd_enum[2],
			msm_quat_mi2s_rx_sample_rate_get, msm_quat_mi2s_rx_sample_rate_put),
#endif
#ifdef CONFIG_SND_SOC_QUIN_HIFI
	...
	SOC_ENUM_EXT("QUIN_MI2S_RX SampleRate", custom_snd_enum[2],
			msm_quin_mi2s_rx_sample_rate_get, msm_quin_mi2s_rx_sample_rate_put),
#endif
#ifdef CONFIG_SND_SOC_TAS_SPK_PROT
	SOC_ENUM_EXT("QUIN_MI2S_TX Channels", msm_snd_enum[1],
			msm_quin_mi2s_tx_ch_get, msm_quin_mi2s_tx_ch_put),
#endif
};
#endif
...
static int msm8952_dmic_event(struct snd_soc_dapm_widget *w,
			      struct snd_kcontrol *kcontrol, int event)
{
#if 0 //fix function. ref config_hph_compander_gpio
...
#if defined(CONFIG_SND_LGE_PD_SWITCH)
	dp_sw = of_get_named_gpio(card->dev->of_node, "anx,dp_sw", 0);
	if(dp_sw > 0 ){
		gpio_request(dp_sw, "dp_sw");
		dp_wifi = __gpio_get_value(dp_sw);
		printk(KERN_INFO"%s : anx dp_sw: %d, dp_wifi: %d\n", __func__, dp_sw, dp_wifi);
	}
...
static int msm_quin_mi2s_snd_startup(struct snd_pcm_substream *substream)
{
	...
	if (atomic_inc_return(&quin_mi2s_clk_ref) == 1) {
		...
		if(!mi2s_quin_master){
			...
			ret = snd_soc_dai_set_fmt(cpu_dai, SND_SOC_DAIFMT_CBS_CFS);
#if defined(CONFIG_SND_SOC_TAS2560) || defined(CONFIG_SND_SOC_TAS2562)
			// Smart Pa handle fmt values in the driver
#else
...
#ifdef CONFIG_MACH_LGE
	snd_soc_add_codec_controls(ana_cdc, custom_snd_controls,
			ARRAY_SIZE(custom_snd_controls));
#endif
...
static struct snd_soc_dai_link_component multi_codecs[] = {
	{
		.name = "cs35l41-codec.6-0040",
		.dai_name = "cs35l41-pcm"
	},
	{
		.name = "cs35l41-codec.6-0041",
		.dai_name = "cs35l41-pcm"
	},
};
...
#ifndef CONFIG_SND_SOC_USE_QUIN_MI2S
	{
...
#if defined(CONFIG_SND_SOC_AK4376)
...
#elif defined(CONFIG_SND_SOC_TAS2560) || defined(CONFIG_SND_SOC_TAS2562)
static struct snd_soc_dai_link msm8952_ti_quin_dai_link[] = {
	{
		.name = LPASS_BE_QUIN_MI2S_RX,
		.stream_name = "Quinary MI2S Playback",
		.cpu_dai_name = "msm-dai-q6-mi2s.4",
		.platform_name = "msm-pcm-routing",
		.codec_name     = TAS_CODEC_NAME,
		.codec_dai_name = TAS_DAI_NAME,
		.no_pcm = 1,
		.dpcm_playback = 1,
		.id = MSM_BACKEND_DAI_QUINARY_MI2S_RX,
		#ifdef CONFIG_SND_SOC_QUIN_HIFI
...
#if defined(CONFIG_SND_SOC_CS35L41)
static struct snd_soc_dai_link msm8952_cs_quat_dai_link[] = {
	{
		.name = LPASS_BE_QUAT_MI2S_RX,
		.stream_name = "Quaternary MI2S Playback",
		.cpu_dai_name = "msm-dai-q6-mi2s.3",
...
#elif defined(CONFIG_SND_SOC_TAS2560) || defined(CONFIG_SND_SOC_TAS2562)
ARRAY_SIZE(msm8952_ti_quin_dai_link) +
#else
ARRAY_SIZE(msm8952_hdmi_dba_dai_link) +
#endif
#if defined(CONFIG_SND_SOC_CS35L41)
ARRAY_SIZE(msm8952_cs_quat_dai_link) +
#endif
...
codec_dai:
		...
		if (dai_link[i].codecs && (dai_link[i].num_codecs > 0)) {
			int j;
			for (j = 0; (j < dai_link[i].num_codecs) && (dai_link[i].codecs[j].name); j++) {
				...
				dai_link[i].codecs[j].of_node = phandle;
...
#endif
	{
		memcpy(dailink + len1, msm8952_ti_quin_dai_link,
				sizeof(msm8952_ti_quin_dai_link));
		len1 += ARRAY_SIZE(msm8952_ti_quin_dai_link);
	}
#else
	if (of_property_read_bool(dev->of_node,
...
#endif /* CONFIG_SND_SOC_AK4376 */
#if defined(CONFIG_SND_SOC_CS35L41)
	memcpy(dailink + len1, msm8952_cs_quat_dai_link,
			sizeof(msm8952_cs_quat_dai_link));
	dev_dbg(dev, "%s(): msm8952_cs_quat_dai_link\n",
					__func__);
	len1 += ARRAY_SIZE(msm8952_cs_quat_dai_link);
#endif
	if (of_property_read_bool(dev->of_node,
				"qcom,split-a2dp")) {
...
parse_mclk_freq:
...
#endif
#if defined(CONFIG_SND_LGE_PD_SWITCH)
	dp_sw = of_get_named_gpio(pdev->dev.of_node, "anx,dp_sw", 0);
	if(dp_sw > 0 ){
		gpio_request(dp_sw, "dp_sw");
		printk(KERN_INFO"%s : anx dp_sw: %d", __func__, dp_sw);
	}
#endif
	...
	if(lge_get_smart_pa()){
#if defined(CONFIG_SND_RENAME_CARD)
		sprintf(init_card_name,"%s-extpa",card->name);
		card->name = init_card_name;
#endif
	}
#endif
	/* initialize timer */
	...
	card->dapm_widgets = cs35l41_widgets;
	card->num_dapm_widgets = ARRAY_SIZE(cs35l41_widgets);
	card->dapm_routes = cs35l41_audio_paths;
	card->num_dapm_routes = ARRAY_SIZE(cs35l41_audio_paths);
	card->controls = cs35l41_controls;
	card->num_controls = ARRAY_SIZE(cs35l41_controls);
#endif
