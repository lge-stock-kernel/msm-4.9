...
static inline void setup_switch(struct switch_dev *sdev, struct gpio_button_data *bdata, const char *wakelock_name)
{
	if (sdev == NULL || sdev->name == NULL || !check_gpio_button_data(bdata))
		return;
...
#define SETUP_SWITCH(s_name) setup_switch(&sdev_##s_name, bdata, #s_name "_wakelock")
