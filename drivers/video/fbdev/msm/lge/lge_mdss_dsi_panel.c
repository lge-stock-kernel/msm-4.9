			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	...
	if (data) {
		snprintf(ctrl_pdata->lge_extra.panel_type, sizeof(ctrl_pdata->lge_extra.panel_type), data);
	} else {
		snprintf(ctrl_pdata->lge_extra.panel_type, sizeof(ctrl_pdata->lge_extra.panel_type), "UNDEFINED");
		pr_err("%s: panel_type not specified\n", __func__);
	}

	rc = of_property_read_u32(np, "lge,cabc-status", &tmp);
	ctrl_pdata->lge_extra.cabc_status = (!rc ? tmp : 0);
	...
	if (lge_get_board_rev_no() < HW_REV_1_0)
		ctrl_pdata->lge_extra.cabc_status = 0;
#endif
	pr_info("%s:cabc-status : %d\n", __func__, ctrl_pdata->lge_extra.cabc_status);

	parse_dt_extra_dcs_cmds(np, ctrl_pdata);

	data = of_get_property(np, "lge,ddic", NULL);
	if (data) {
		ctrl_pdata->lge_extra.ddic_ops = find_ddic_ops(data);
		if (ctrl_pdata->lge_extra.ddic_ops)
			pr_info("driver for %s is found\n", data);
	}
...
struct dsi_panel_cmds *lge_get_extra_cmds_by_name(struct mdss_dsi_ctrl_pdata *ctrl_pdata, char *name)
{
	...
	for (i = 0; i < ctrl_pdata->lge_extra.num_extra_cmds; ++i) {
		if (!strcmp(ctrl_pdata->lge_extra.extra_cmds_array[i].name, name))
			return &ctrl_pdata->lge_extra.extra_cmds_array[i].cmds;
	}
	return NULL;
}
...
__weak int lge_mdss_dsi_screen_color_mode_set(struct mdss_dsi_ctrl_pdata *ctrl)
{
	if (ctrl == NULL)
		return -EINVAL;

	switch (ctrl->panel_data.panel_info.screen_color_mode) {
	case LGE_COLOR_OPT:
	case LGE_COLOR_CIN:
	case LGE_COLOR_SPO:
	case LGE_COLOR_GAM:
	case LGE_COLOR_MAN:
		LGE_DDIC_OP(ctrl, screen_color_mode_set, ctrl->panel_data.panel_info.screen_color_mode);
		break;
	default:
		return -EINVAL;
	}
...
__weak int lge_mdss_dsi_rgb_tune_set(struct mdss_dsi_ctrl_pdata *ctrl)
{
	struct mdss_panel_info *pinfo = NULL;
	...
	CHECK_BOUNDARY(pinfo->cm_preset_step, 0, 5);
	CHECK_BOUNDARY(pinfo->cm_red_step, 0, 5);
	CHECK_BOUNDARY(pinfo->cm_green_step, 0, 5);
	CHECK_BOUNDARY(pinfo->cm_blue_step, 0, 5);
	LGE_DDIC_OP(ctrl, rgb_tune_set, pinfo->cm_preset_step, pinfo->cm_red_step, pinfo->cm_green_step, pinfo->cm_blue_step);

	return 0;
}

__weak int lge_mdss_dsi_screen_tune_set(struct mdss_dsi_ctrl_pdata *ctrl)
{
	struct mdss_panel_info *pinfo = NULL;
	...
	pinfo = &ctrl->panel_data.panel_info;
	CHECK_BOUNDARY(pinfo->sc_sat_step, 0, 5);
	CHECK_BOUNDARY(pinfo->sc_hue_step, 0, 5);
	CHECK_BOUNDARY(pinfo->sc_sha_step, 0, 5);
	LGE_DDIC_OP(ctrl, screen_tune_set, pinfo->sc_sat_step, pinfo->sc_hue_step, pinfo->sc_sha_step);
