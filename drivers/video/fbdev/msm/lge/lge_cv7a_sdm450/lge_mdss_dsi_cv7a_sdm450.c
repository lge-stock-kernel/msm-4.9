...
int lge_mdss_dsi_rgb_tune_set(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	if (pinfo->cm_red_step > DG_MODE_MAX)
		pinfo->cm_red_step = DG_MODE_MAX;
	if (pinfo->cm_green_step > DG_MODE_MAX)
		pinfo->cm_green_step = DG_MODE_MAX;
	if (pinfo->cm_blue_step > DG_MODE_MAX)
		pinfo->cm_blue_step = DG_MODE_MAX;
...
int lge_mdss_dsi_screen_tune_set(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	if (pinfo->sc_sat_step > SC_MODE_MAX)
		pinfo->sc_sat_step = SC_MODE_MAX;
	if (pinfo->sc_hue_step > SC_MODE_MAX)
		pinfo->sc_hue_step = SC_MODE_MAX;
	if (pinfo->sc_sha_step > SC_MODE_MAX)
		pinfo->sc_sha_step = SC_MODE_MAX;
...
int lge_mdss_dsi_screen_color_mode_set(struct mdss_dsi_ctrl_pdata *ctrl)
{
	...
	int eco_cabc_val = ECO_CABC_DEFAULT;
	char cabc_cmd_name[256];
	...
	switch (pinfo->screen_color_mode) {
		case LGE_COLOR_OPT:
			break;
//		case LGE_COLOR_ECO:	/* TBD */
//			eco_cabc_val = ECO_CABC_50;
//			break;
		case LGE_COLOR_CIN:
    ...
    pr_info("%s gc_mode=[%d], eco_cabc_val=[%d] \n", __func__, gc_mode, eco_cabc_val);
    pinfo->gc_mode = gc_mode;

    snprintf(cabc_cmd_name, sizeof(cabc_cmd_name),"cabc-%d", eco_cabc_val);
    lge_mdss_dsi_panel_extra_cmds_send(NULL, cabc_cmd_name);
