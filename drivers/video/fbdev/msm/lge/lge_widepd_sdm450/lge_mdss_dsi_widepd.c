...
void lge_panel_enter_deep_sleep(void)
{
	switch (lge_get_panel_id()) {
		case ANX7625_WIDEPD:
			anx7625_widepd_panel_enter_deep_sleep();
			break;
		default:
			break;
	}
...
void lge_panel_exit_deep_sleep(void)
{
	...
	switch (lge_get_panel_id()) {
		case ANX7625_WIDEPD:
			anx7625_widepd_panel_exit_deep_sleep();
			break;
		default:
			break;
	}
...
int mdss_dsi_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	switch (lge_get_panel_id()) {
		case ANX7625_WIDEPD:
			rc = anx7625_widepd_mdss_dsi_panel_reset(pdata, enable);
			break;
		default:
			break;
        }
...
int mdss_dsi_panel_power_on(struct mdss_panel_data *pdata)
{
	...
	switch (lge_get_panel_id()) {
		case ANX7625_WIDEPD:
			ret = anx7625_widepd_mdss_dsi_panel_power_on(pdata);
			break;
		default:
			break;
       }
...
int mdss_dsi_panel_power_off(struct mdss_panel_data *pdata)
{
	...
	switch (lge_get_panel_id()) {
		case ANX7625_WIDEPD:
			ret = anx7625_widepd_mdss_dsi_panel_power_off(pdata);
			break;
		default:
			break;
       }
...
void mdss_dsi_ctrl_shutdown(struct platform_device *pdev)
{
	switch (lge_get_panel_id()) {
		case ANX7625_WIDEPD:
			anx7625_widepd_mdss_dsi_ctrl_shutdown(pdev);
			break;
		default:
			break;
	}
...
int lge_mdss_dsi_pre_event_handler(struct mdss_panel_data *pdata, int event, void *arg)
{
	...
	case MDSS_EVENT_BLANK:
		...
		rc = mip4_fb_notifier_call_chain(FB_BLANK_POWERDOWN, (void *)&event);
		//anx power off
		break;
...
int lge_mdss_dsi_post_event_handler(struct mdss_panel_data *pdata, int event, void *arg)
{
	...
	case MDSS_EVENT_POST_PANEL_ON:
		...
		//anx power on
		rc = mip4_fb_notifier_call_chain(FB_BLANK_UNBLANK, (void *)&event);
		break;
	case MDSS_EVENT_BLANK:
		break;
	case MDSS_EVENT_PANEL_OFF:
