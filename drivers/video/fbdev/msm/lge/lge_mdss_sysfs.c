...
static char *strhexbuf = NULL;
static int strhex(const char* in, int *value)
{
	char str[3];
	...
	while (strhexbuf && strlen(strhexbuf) && *strhexbuf == ' ') strhexbuf++;
	...
	*value = 0;
	pr_info("strhex: %s\n", str);
	return kstrtouint(str, 16, value);
}

static ssize_t oncmd_set(struct device *dev, struct device_attribute *attr, const char *buf, size_t len)
{
	ssize_t ret = strnlen(buf, PAGE_SIZE);
	struct fb_info *fbi = dev_get_drvdata(dev);
	struct msm_fb_data_type *mfd = (struct msm_fb_data_type *)fbi->par;
	struct mdss_panel_data *pdata = dev_get_platdata(&mfd->pdev->dev);
	struct mdss_dsi_ctrl_pdata *ctrl = container_of(pdata, struct mdss_dsi_ctrl_pdata, panel_data);
	struct dsi_panel_cmds *on_cmd = &ctrl->on_cmds;
	struct dsi_cmd_desc *cmd = NULL;
	int value = 0;
	int i = 0;

	if (strhex(buf, &value) == 0) {
		cmd = find_cmd(on_cmd->cmds, on_cmd->cmd_cnt, value);
	}

	if (cmd) {
		i = 1;
		while (strhex(NULL, &value) == 0 && i < cmd->dchdr.dlen) {
			cmd->payload[i++] = value;
		}
	}
	return ret;
...
static DEVICE_ATTR(ovwr_oncmd, S_IWUSR|S_IWGRP, NULL, oncmd_set);
