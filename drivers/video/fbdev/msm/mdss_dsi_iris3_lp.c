...
bool iris_pt_to_abypass_switch(struct msm_fb_data_type *mfd)
{
	...
	frame = mdss_panel_get_framerate(
			&(pcfg->ctrl->panel_data.panel_info), FPS_RESOLUTION_HZ);
...
void iris_abyp_lightup_set(int abyp)
{
	debug_abyp_opt = abyp;
}
...
#define MODULE_PARAM_PREFIX "pxlw_iris3."
static int param_set_iris_abyp(const char *val, const struct kernel_param *kp)
{
	int buf;
	int ret = kstrtoint(val, 0, &buf);
	if (ret)
		return ret;
	debug_abyp_opt = buf;
	pr_info("module parameter pxlw_iris3.abyp = %d\n", debug_abyp_opt);
	return 0;
}
static int param_get_iris_abyp(char *buf, const struct kernel_param *kp)
{
	return scnprintf(buf, PAGE_SIZE-1, "%d", debug_abyp_opt);
}
static struct kernel_param_ops iris_abyp_ops = {
	.set = param_set_iris_abyp,
	.get = param_get_iris_abyp,
};
module_param_cb(abyp, &iris_abyp_ops, NULL, S_IRUGO);
