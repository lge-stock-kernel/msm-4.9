...
extern void lge_report_iris_ready(void);
...
//#define USE_LK
	...
	for (i = 0; i < num; i++) {
		...
		sprintf(str, "0x%02x", (uint8_t)dlen);
		pr_err("[LK_CMD]  %s, 0x00, 0x29, 0xC0,\n", str);

		for (j = 0; j < dlen; j++) {
			snprintf(temp_buf+strlen(temp_buf), sizeof(temp_buf)-strlen(temp_buf), "0x%02x, ", pcmd->payload[j]);
			if (j % 4 == 3) {
				pr_err("[LK_CMD]  %s\n", temp_buf);
				temp_buf[0] = '\0';
			}
