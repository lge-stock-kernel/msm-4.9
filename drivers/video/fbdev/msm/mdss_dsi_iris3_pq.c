...
extern void dw8831_lcd_backlight_set_level(int level);
...
void iris_panel_nits_set(u32 bl_ratio, bool bSystemRestore, int level)
{
	...
	if (bSystemRestore)
		...
		bl_level = 2047; //Fixed for now //bl_ratio * pcfg->panel_dimming_brightness / PANEL_BL_MAX_RATIO;
	...
	case BL_DCS_CMD:
		...
		break;
	case BL_OTHERS:
		pr_err("%s-BL_OTHERS: bl_level=%d\n", __func__, bl_level);
		dw8831_lcd_backlight_set_level(bl_level);
		break;
	default:
