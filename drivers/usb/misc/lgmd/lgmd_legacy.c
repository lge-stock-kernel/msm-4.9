...
#define ADC_CC_CHANGED_TIME     (2000 * HZ/1000) /* 2000 msec */
...
static int cc_off_polling = 10;
module_param(cc_off_polling, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(cc_off_polling, "CC Off Polling Delay");
...
static enum alarmtimer_restart lge_moisture_timer_func(struct alarm *alarm,
							ktime_t now)
{
	struct lge_moisture *moist = container_of(alarm, struct lge_moisture, timer);
...
static void lge_moist_sbu_adc_work(struct work_struct *w)
{
	...
	case ADC_STATE_START_WET_DETECT:
		...
		if (vadc_physical < sbu_adc_gnd_low_threshold) {
			...
			work = 1;
		} else if ((!moist->force_pr_sink && !moist->sbu_moisture) ||
	...
	case ADC_STATE_CC_STOP_ONLY:
		if (moist->force_pr_sink) {
			...
			moist->sbu_run_work = false;
			work = 1;
			delay = (cc_off_polling * HZ);
			alarm_start_relative(&moist->timer, ms_to_ktime(cc_off_polling * 1000));
	...
	case ADC_STATE_WET:
		...
		if (moist->sbu_adc_state == ADC_STATE_WET) {
			...
			alarm_start_relative(&moist->timer, ms_to_ktime(10 * 1000));
	...
	if (work) {
		...
		if (moist->method != MOISTURE_EDGE_PD_SBU_PU || moist->sbu_moisture) {
			if (moist->sbu_adc_param.low_thr == moist->sbu_adc_param.high_thr) {
				if (moist->sbu_adc_param.state_request == ADC_TM_LOW_THR_ENABLE) {
					moist->sbu_adc_param.high_thr = 1800000;
				} else if (moist->sbu_adc_param.state_request == ADC_TM_HIGH_THR_ENABLE) {
					moist->sbu_adc_param.low_thr = 0;
				}
			}
...
struct lge_moisture *lge_moisture_create(struct device *parent)
{
	...
	alarm_init(&moist->timer, ALARM_BOOTTIME, lge_moisture_timer_func);
