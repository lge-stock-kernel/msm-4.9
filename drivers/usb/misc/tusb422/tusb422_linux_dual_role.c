									  ...
									  unsigned int *val)
{
	...
	case DUAL_ROLE_PROP_MODE:
		if (tcpc_dev->flags & TC_FLAGS_TEMP_ROLE)
			...
			*val = tcpc_dev->moisture_detect_ux ? DUAL_ROLE_PROP_MODE_FAULT
	...
	case DUAL_ROLE_PROP_PR:
		if (tcpc_dev->flags & TC_FLAGS_TEMP_ROLE)
			...
			*val = tcpc_dev->moisture_detect_ux ? DUAL_ROLE_PROP_PR_FAULT
				: DUAL_ROLE_PROP_MODE_FAULT_NO_UX;
#endif
		} else
	...
	case DUAL_ROLE_PROP_MOISTURE_UX:
		*val = tcpc_dev->moisture_detect_ux ? DUAL_ROLE_PROP_MOISTURE_UX_ENABLE
									  ...
									  const unsigned int *val)
{
	...
	case DUAL_ROLE_PROP_MOISTURE_UX:
		if (*val == DUAL_ROLE_PROP_MOISTURE_UX_ENABLE) {
			tcpc_dev->moisture_detect_ux = true;
		} else if (*val == DUAL_ROLE_PROP_MOISTURE_UX_DISABLE) {
			tcpc_dev->moisture_detect_ux = false;
		}
			pval.intval = tcpc_dev->moisture_detect_ux;
		power_supply_set_property(tcpc_dev->typec_psy,
