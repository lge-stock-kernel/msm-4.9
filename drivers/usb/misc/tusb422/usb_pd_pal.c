...
#define RDO_USB_COMM_CAPABLE_BIT ((uint32_t)1 << 25)
#define PDO_USB_COMM_CAPABLE_BIT ((uint32_t)1 << 26)
#endif
...
#ifdef CONFIG_LGE_USB_TYPE_C
			pd_state.usb_comm = pd_dev->offered_rdo & RDO_USB_COMM_CAPABLE_BIT;
#endif
...
#ifdef CONFIG_LGE_USB_TYPE_C
			pd_state.usb_comm = pd_dev->offered_pdo[0] & PDO_USB_COMM_CAPABLE_BIT;
#endif
			pd_dpm_handle_pe_event(PD_DPM_PE_EVT_PD_STATE, (void *)&pd_state);
			break;
...
void usb_pd_pal_notify_connect_state(unsigned int port, tcpc_state_t state, bool polarity)
{
...
#ifdef CONFIG_LGE_USB_TYPE_C
	tc_state.typec_mode = POWER_SUPPLY_TYPEC_NONE;
#endif
	...
	switch (state)
	{
		...
		case TCPC_STATE_UNATTACHED_SNK:
			...
			if(!support_MoistureDetect()) // CV7AS ATT support moisture detect
			{
...
#ifdef CONFIG_LGE_USB_TYPE_C
			if (tcpc_dev->plug_polarity == PLUG_UNFLIPPED) {
				cc1 = TCPC_CC1_STATE(tcpc_dev->cc_status);
				cc2 = TCPC_CC2_STATE(tcpc_dev->cc_status);
			} else {
				cc1 = TCPC_CC2_STATE(tcpc_dev->cc_status);
				cc2 = TCPC_CC1_STATE(tcpc_dev->cc_status);
			}

			if (cc1 == CC_SRC_STATE_RD) {
				if (cc2 == CC_SRC_STATE_OPEN)
					tc_state.typec_mode = POWER_SUPPLY_TYPEC_SINK;
				else if (cc2 == CC_SRC_STATE_RA)
					tc_state.typec_mode = POWER_SUPPLY_TYPEC_SINK_POWERED_CABLE;
				else if (cc2 == CC_SRC_STATE_RD)
					tc_state.typec_mode = POWER_SUPPLY_TYPEC_SINK_DEBUG_ACCESSORY;
			} else if (cc1 == CC_SRC_STATE_RA && cc2 == CC_SRC_STATE_RA) {
				tc_state.typec_mode = POWER_SUPPLY_TYPEC_SINK_AUDIO_ADAPTER;
			} else if (cc1 == CC_SRC_STATE_RA || cc2 == CC_SRC_STATE_RA) {
				tc_state.typec_mode = POWER_SUPPLY_TYPEC_POWERED_CABLE_ONLY;
			}
#endif
			pd_dpm_handle_pe_event(PD_DPM_PE_EVT_TYPEC_STATE, (void*)&tc_state);
			break;
		...
		case TCPC_STATE_DEBUG_ACC_SNK:
			...
			if (state == TCPC_STATE_DEBUG_ACC_SNK)
				tc_state.typec_mode = POWER_SUPPLY_TYPEC_SINK_DEBUG_ACCESSORY;
			else if (!(tcpc_dev->cc_status & CC_STATUS_CONNECT_RESULT))
				tc_state.typec_mode = POWER_SUPPLY_TYPEC_NON_COMPLIANT;
			else if (cc1 == CC_SNK_STATE_DEFAULT)
				tc_state.typec_mode = POWER_SUPPLY_TYPEC_SOURCE_DEFAULT;
			else if (cc1 == CC_SNK_STATE_POWER15)
				tc_state.typec_mode = POWER_SUPPLY_TYPEC_SOURCE_MEDIUM;
			else if (cc1 == CC_SNK_STATE_POWER30)
				tc_state.typec_mode = POWER_SUPPLY_TYPEC_SOURCE_HIGH;
			else
				tc_state.typec_mode = POWER_SUPPLY_TYPEC_NON_COMPLIANT;
#endif
			pd_dpm_handle_pe_event(PD_DPM_PE_EVT_TYPEC_STATE, (void*)&tc_state);
...
#ifdef CONFIG_LGE_USB_MOISTURE_DETECT
int usb_pd_pal_get_sbu_adc(unsigned int port, int num)
{
	return pd_dpm_handle_pe_event(PD_DPM_PE_EVENT_GET_SBU_ADC, (void *)&num);
}
