...
{
	if(_hw_pd_dev.sbu_sw_sel1_gpio) {
		if(enable) {
			/* SBU_SW_SEL_1 Low -> UART */
			gpiod_direction_output(_hw_pd_dev.sbu_sw_sel1_gpio, 0);
			usleep_range(40000, 41000); /* Turn-On Time /OE */
		}
		else {
			/* SBU_SW_SEL_1 High -> AUX */
			gpiod_direction_output(_hw_pd_dev.sbu_sw_sel1_gpio ,1);
			usleep_range(4000, 4100); /* Turn-Off Time /OE */
		}
	}
	else
		PRINT("%s: failed to get sbu_sw_sel1 gpio\n", __func__);
}
EXPORT_SYMBOL(handle_uart_switch);

void handle_aux_mux_switch(int polarity)
{
	int aux, mux, rc;

	/* polarity = 0 : UNFLIPPED, 1 : FLIPPED */
	if (polarity == UNFLIPPED) {
		/* REDERIVER_SEL High, SBU_SW_SEL2(Aux switch Sel) High */
		mux = gpiod_direction_output(_hw_pd_dev.redriver_sel_gpio, 1);
		aux = gpiod_direction_output(_hw_pd_dev.sbu_sw_sel2_gpio, 1);

	}
	else { // FLIPPED
		/* REDERIVER_SEL Low, SBU_SW_SEL2(Aux switch Sel) Low */
		mux = gpiod_direction_output(_hw_pd_dev.redriver_sel_gpio, 0);
		aux = gpiod_direction_output(_hw_pd_dev.sbu_sw_sel2_gpio, 0);
	}

	/* REDRIVER_EN Output enable */
	rc = gpiod_direction_output(_hw_pd_dev.redriver_en_gpio, 0);
	usleep_range(10000, 11000); /* Turn-On Time /OE */

	PRINT("%s: polarity %s, mux %d, aux %d, rc %d",
			__func__, polarity?"FLIPPED":"UNFLIPPED", mux, aux, rc);
}
EXPORT_SYMBOL(handle_aux_mux_switch);

void handle_dp_connection(int status)
{
	struct hw_pd_dev *dev = &_hw_pd_dev;
	char *dp_disconnected[2] = { "DP_STATE=DISCONNECTED", NULL };
	char *dp_connected[2]    = { "DP_STATE=CONNECTED", NULL };

	// change mux patch for display
	anx7625_set_mux_path(status);

	if (status) // connected
	{
		mdelay(5);
		// send uevent for DP connection HID touch
		kobject_uevent_env(&dev->dev->kobj, KOBJ_CHANGE, dp_connected);
		PRINT("%s: sent uevent %s\n", __func__, dp_connected[0]);
	}
	else // disconnected
	{
		kobject_uevent_env(&dev->dev->kobj, KOBJ_CHANGE, dp_disconnected);
		PRINT("%s: sent uevent %s\n", __func__, dp_disconnected[0]);
	}
}
EXPORT_SYMBOL(handle_dp_connection);
#endif
...
int hw_pd_dev_init(struct device *dev)
{
	...
	_hw_pd_dev.redriver_en_gpio = devm_gpiod_get(dev, "ti,redriver-en",
						GPIOD_OUT_LOW);
	if (IS_ERR(_hw_pd_dev.redriver_en_gpio)) {
		PRINT("failed to allocate redriver_en gpio\n");
		_hw_pd_dev.redriver_en_gpio = NULL;
	}
	_hw_pd_dev.sbu_sw_sel1_gpio = devm_gpiod_get(dev, "ti,sbu-sw-sel1",
						GPIOD_OUT_LOW);
	if (IS_ERR(_hw_pd_dev.sbu_sw_sel1_gpio)) {
		PRINT("failed to allocate sbu_sw_sel1 gpio\n");
		_hw_pd_dev.sbu_sw_sel1_gpio = NULL;
	}
	_hw_pd_dev.sbu_sw_sel2_gpio = devm_gpiod_get(dev, "ti,sbu-sw-sel2",
						GPIOD_OUT_LOW);
	if (IS_ERR(_hw_pd_dev.sbu_sw_sel2_gpio)) {
		PRINT("failed to allocate sbu_sw_sel2 gpio\n");
		_hw_pd_dev.sbu_sw_sel2_gpio = NULL;
	}
	_hw_pd_dev.vreg_fusb_en = devm_regulator_get(dev, "ti,vreg18");
	if (IS_ERR(_hw_pd_dev.vreg_fusb_en)) {
		PRINT("failed to get vreg18 supply\n");
		return PTR_ERR(_hw_pd_dev.vreg_fusb_en);
	}
	if (_hw_pd_dev.vreg_fusb_en) {
		rc = regulator_enable(_hw_pd_dev.vreg_fusb_en);
		if(rc < 0)
			PRINT("Failed to enable vreg18 supply %d\n", rc);
	}
#endif
