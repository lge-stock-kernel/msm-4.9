				   ...
				   const struct kernel_param *kp)
{
	...
	if (tusb422_pd) {
		if (usb_compliance_mode) {
			__pm_stay_awake(&tusb422_pd->wake_lock);
			pm_qos_add_request(&tusb422_pd->pm_qos, PM_QOS_CPU_DMA_LATENCY, 0);
		} else {
			pm_qos_remove_request(&tusb422_pd->pm_qos);
			__pm_relax(&tusb422_pd->wake_lock);
		}
	}
...
#if defined(CONFIG_PM_WAKELOCKS) && defined(CONFIG_LGE_USB_TYPE_C)
	wake_lock_timeout(&tusb422_pd->attach_wakelock,
			  msecs_to_jiffies(WAKE_LOCK_TIMEOUT_MS));
#endif

#ifdef CONFIG_LGE_USB_TYPE_C
	tusb422_pwr->work.func(&tusb422_pwr->work);
#else
...
static void tusb422_start_work(struct work_struct *work)
{
	...
	usb_pd_check_disable_high_voltage();
#endif
#endif
...
#ifdef CONFIG_LGE_USB_TYPE_C
	tusb422_pd->configuration->usb_compliance_mode = &usb_compliance_mode;
#endif
#ifdef CONFIG_LGE_USB_MOISTURE_DETECT
#ifdef CONFIG_LGE_USB_FACTORY
	if (IS_FACTORY_MODE)
		...
		if(support_MoistureDetect()) // CV7AS ATT support moisture detect
		    tusb422_pd->configuration->moisture_detect_disable = false;
		else
			tusb422_pd->configuration->moisture_detect_disable = true;
#endif
							 ...
							 const struct i2c_device_id *id)
{
	...
	int ret;
	dev_err(dev, "%s: tusb422_i2c_probe ###eunmi\n", __func__);
...
#ifdef CONFIG_LGE_USB_TYPE_C
	wakeup_source_init(&tusb422_pd->wake_lock, "usb_compliance_mode");
#endif
