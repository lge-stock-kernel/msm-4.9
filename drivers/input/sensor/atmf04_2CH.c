...
static struct attribute *atmf04_attributes_input[] = {
	&dev_attr_onoff.attr,
	NULL,
};
...
static struct attribute_group atmf04_attr_group_input = {
	.attrs = atmf04_attributes_input,
};
...
//	wake_lock_init(&data->ps_wlock, WAKE_LOCK_SUSPEND, "capsensor_wakelock");
	...
	err = sysfs_create_group(&data->input_dev_cap->dev.kobj, &atmf04_attr_group_input);
	if (err)
