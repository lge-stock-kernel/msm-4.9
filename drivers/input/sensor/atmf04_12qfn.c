...
#define ATMF04_CR_DUTY_LOW        128 // CR range MIN
#define ATMF04_CR_DUTY_HIGH       1152 // CR Range MAX
#define ATMF04_CS_DUTY_LOW        128 // CS Range MIN
#define ATMF04_CS_DUTY_HIGH       1152 // CS Range MAX
...
#define ATMF04_CSCR_RESULT          -2
#endif // defined(CONFIG_MACH_MSM8917_MH5LM, MH5LM_NONAB)
...
static unsigned char load_firmware(struct atmf04_data *data, struct i2c_client *client, const char *name)
{
  ...
  chip_id= i2c_smbus_read_byte_data(client, 0x00);
    ...
    {
	if((chip_id == 0xb4)||(chip_id == 0x00))
	{
	    PINFO(" ATMF04 SAR controller ATMF04 chip_id:0x%x , cnt:%d\n", chip_id, i);
	    break;
	}
	chip_id= i2c_smbus_read_byte_data(client, 0x00);
	if( i == IC_TIMEOUT_CNT-1)
	{
	    PINFO(" ATMF04 SAR controller ATMF04 chip_id read fail chip_id:0x%x\n", chip_id);
	    if (fw)
		release_firmware(fw);
	    return 1;
	}
...
#if 0 // debugging calibration paused
...
static int atmf04_pm_resume(struct device *dev)
{
	...
	if (atomic_read(&pm_suspend_flag) == DEV_PM_SUSPEND_IRQ) {
		if (gpio_get_value(data->platform_data->chip_enable) == 0) { // if power on
			PINFO("ATMF04 queue_delayed_work now.");
