...
extern dma_addr_t g_dma_buff_pa;
#endif

extern int ft5446_reg_read(struct device *dev, u16 addr, void *data, int size);
extern int ft5446_reg_write(struct device *dev, u16 addr, void *data, int size);
...
*******************************************************************************/
	...
	int fts_i2c_read(struct i2c_client *client, char *writebuf,int writelen, char *readbuf, int readlen)
	{
		...
		if((NULL!=client) && (writelen>0) && (writelen<=128))
		{
			...
			if((ret=i2c_master_send(client,(void *)(uintptr_t)g_dma_buff_pa, writelen))!=writelen)
				//dev_err(&client->dev, "###%s i2c write len=%x,buffaddr=%x\n", __func__,ret,*g_dma_buff_pa);
				printk("i2c write failed\n");
			client->addr = (client->addr & I2C_MASK_FLAG) &(~ I2C_DMA_FLAG);
#else
			if((ret=i2c_master_send(client,(void *)writebuf, writelen))!=writelen)
				printk("i2c write failed\n");
#endif
		}
	...
	
		{
			...
			ret = i2c_master_recv(client, (void *)(uintptr_t)g_dma_buff_pa, readlen);
	
			memcpy(readbuf, g_dma_buff_va, readlen);
	
			client->addr = (client->addr & I2C_MASK_FLAG) &(~ I2C_DMA_FLAG);
#else
			ret = i2c_master_recv(client, (void *)readbuf, readlen);
#endif
		}
...
*******************************************************************************/
	...
	int fts_i2c_write(struct i2c_client *client, char *writebuf, int writelen)
	{
		...
		else if((writelen > 8)&&(NULL != tpd_i2c_dma_va))
		{
			...
			client->addr = (client->addr & I2C_MASK_FLAG )| I2C_DMA_FLAG;
			//client->ext_flag = client->ext_flag | I2C_DMA_FLAG;
			ret = i2c_master_send(client, (void *)(uintptr_t)tpd_i2c_dma_pa, writelen);
			client->addr = client->addr & I2C_MASK_FLAG & ~I2C_DMA_FLAG;
...
#else
		ret = i2c_master_send(client, (void *)writebuf, writelen);
#endif
		return ret;
...
{
	struct fts_rw_i2c_queue i2c_rw_queue = {0,};
