...
#ifndef LGE_TOUCH_FT5446_H
#define LGE_TOUCH_FT5446_H

#define FT5446_ESD_SKIP_WHILE_TOUCH_ON
/* focaltech APK debug */
...
struct ft5446_touch_info {
	...
	u32 button_cnt:3;
	u32 palm_bit:16;
	struct ft5446_touch_data data[10];
	/* debug info */
	struct ft5446_touch_debug debug;
} __packed;
...
struct ft5446_ic_info {
	struct ft5446_version version;
	u8 info_valid;
	u8 chip_id;
	u8 chip_id_low;
	u8 fw_vendor_id;
...
struct ft5446_data {
	struct device *dev;
	struct kobject kobj;
	struct ft5446_touch_info info;
	struct ft5446_ic_info ic_info;
//	struct ft5446_asc_info asc;	/* ASC */
...
#ifdef CONFIG_LGD_FT5446_FHD_VIDEO_LCD_PANEL
...
static inline struct ft5446_data *to_ft5446_data(struct device *dev)
{
	return (struct ft5446_data *)touch_get_device(to_touch_core(dev));
}

static inline struct ft5446_data *to_ft5446_data_from_kobj(struct kobject *kobj)
{
	return (struct ft5446_data *)container_of(kobj,
			struct ft5446_data, kobj);
}
...
int ft5446_tc_driving(struct device *dev, int mode);
int ft5446_irq_abs(struct device *dev);
int ft5446_irq_lpwg(struct device *dev, int tci);
int ft5446_irq_handler(struct device *dev);
int ft5446_check_status(struct device *dev);
int ft5446_debug_info(struct device *dev, int mode);
...
static inline int ft5446_read_value(struct device *dev,
					u16 addr, u32 *value)
{
	return ft5446_reg_read(dev, addr, value, sizeof(*value));
}

static inline int ft5446_write_value(struct device *dev,
					 u16 addr, u32 value)
{
	return ft5446_reg_write(dev, addr, &value, sizeof(value));
}

//void ft5446_xfer_msg_ready(struct device *dev, u8 msg_cnt);

extern int get_smartcover_status(void);
extern int ft5446_lpwg_set(struct device *dev);
extern bool is_ddic_name(char *ddic_name);
#endif /* LGE_TOUCH_FT5446_H */
