 ...
 * DOLLARS.
 */
#define TS_MODULE "[td4310_rmi_dev]"
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	/* page read */
	ret = td4310_read(dev, PAGE_SELECT_REG, &page_old, sizeof(page_old));
	...
	if (page_old == page_new) {
		...
		ret = td4310_write(dev, PAGE_SELECT_REG,
	...
	/* read register */
	ret = td4310_read(dev, (u8)(addr & ~(MASK_8BIT << 8)), data, length);
		...
		unsigned short addr, unsigned char *data, unsigned short length)
{
	...
	/* page read */
	ret = td4310_read(dev, PAGE_SELECT_REG,
				&page_old, sizeof(page_old));
	...
	if (page_old == page_new) {
		...
		ret = td4310_write(dev, PAGE_SELECT_REG, &page_new, sizeof(page_new));
	...
	/* write register */
	ret = td4310_write(dev, (u8)(addr & ~(MASK_8BIT << 8)), data, length);
		...
		bool enable)
{
	...
	struct td4310_data *d = to_td4310_data(ts_dev);
	int retval = 0;
...
/*	unsigned char intr_status = 0;	*/
	...
	struct rmidev_data *dev_data = filp->private_data;
/*	struct td4310_rmi4_fn *fhandler;	*/
...
static struct td4310_rmidev_exp_fn rmidev_module = {
...
static int __init rmidev_module_init(void)
{
	...
	td4310_rmidev_function(&rmidev_module, true);
...
static void __exit rmidev_module_exit(void)
{
	...
	td4310_rmidev_function(&rmidev_module, false);
