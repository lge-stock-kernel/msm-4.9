...
static int RspSetCommandType(struct device *dev, u8 setValue, int report_type)
{
	...
	/*Get report*/
	SYNA_WRITE(dev, ANALOG_COMMAND_REG, &setValue, sizeof(setValue), ret, error);

	/*Waiting for complete*/
	do {
		touch_msleep(10);
		SYNA_READ(dev, ANALOG_COMMAND_REG, &setValue, sizeof(setValue), ret, error);
...
static int RspResetReportAddress(struct device *dev)
{
	...
	SYNA_WRITE(dev, RESET_REPORT_INDEX_LSB_REG, &data, 1, ret, error);
	SYNA_WRITE(dev, RESET_REPORT_INDEX_MSB_REG, &data, 1, ret, error);
...
static int RspReadImageReport(struct device *dev, int16_t *buf, int buf_size)
{
	...
	SYNA_READ(dev, READ_REPORT_DATA_REG, (u8 *)buf, buf_size, ret, error);
...
static int td4310_Get_Report_Data(struct device *dev, int report_type, int16_t* result_buf, int buf_size)
{
	...
	data = report_type;
	SYNA_WRITE(dev, SET_REPORT_TYPE_REG, &data, 1, ret, FAIL);
...
FAIL:
	ret2 = td4310_set_page(dev, DEFAULT_PAGE);
	if (ret2 < 0)
...
static int td4310_Get_Delta_Data(struct device *dev)
{
	...
	if (!delta) {
		TOUCH_E("Fail to allocate memory for delta\n");
		return -ENOMEM;
	}
...
static int td4310_Get_Electrode_Open_Data(struct device *dev)
{
	...
	/* Read all the values of ANALOG_CONTROL2_REG */
	SYNA_READ(dev, ANALOG_CONTROL2_REG, original_analog_ctrl2,
			sizeof(original_analog_ctrl2), ret, FAIL); /* F54_ANALOG_CTRL91 */
	/* Backup the values of ANALOG_CONTROL2_REG to be re written at the end */
	memcpy(analog_ctrl2, original_analog_ctrl2, sizeof(original_analog_ctrl2));

	/* Backup the values of CBC_TIMING_CONTROL_REG to be re written at the end */
	SYNA_READ(dev, CBC_TIMING_CONTROL_REG, original_cbc_timing_ctrl,
			sizeof(original_cbc_timing_ctrl), ret, FAIL); /* F54_ANALOG_CTRL99 (Integration Duration) */

	/* Backup the values of Waveform/Reset Duration to be re written at the end */
	SYNA_READ(dev, WAVEFORM_DURATION_CTRL_REG, original_intdur,
			sizeof(original_intdur), ret, FAIL); /* F54_ANALOG_CTRL99 (Integration Duration) */
	...
	SYNA_WRITE(dev, ANALOG_CONTROL2_REG, analog_ctrl2,
			sizeof(analog_ctrl2), ret, FAIL);  /* F54_ANALOG_CTRL91 (Integration Duration) */

	SYNA_READ(dev, WAVEFORM_DURATION_CTRL_REG, intdur_phase1,
			sizeof(intdur_phase1), ret, FAIL); /* F54_ANALOG_CTRL99 (Integration Duration) */
	/* Set Reset Duration */
	intdur_phase1[2] = RESET_DURATION & 0xff;/* Increase RST_DUR to 1.53us */
	SYNA_WRITE(dev, WAVEFORM_DURATION_CTRL_REG, intdur_phase1,
			sizeof(intdur_phase1), ret, FAIL);  /* F54_ANALOG_CTRL99 (Integration Duration) */
	...
	SYNA_WRITE(dev, CBC_TIMING_CONTROL_REG, cbc_timing_ctrl,
			sizeof(cbc_timing_ctrl), ret, FAIL);  /* F54_ANALOG_CTRL182 (CBC TIMING CTRL)*/
	...
	SYNA_READ(dev, WAVEFORM_DURATION_CTRL_REG, intdur_phase2,
			sizeof(intdur_phase2), ret, FAIL); /* F54_ANALOG_CTRL99 (Integration Duration) */
	...
	SYNA_WRITE(dev, WAVEFORM_DURATION_CTRL_REG, intdur_phase2,
			sizeof(intdur_phase2), ret, FAIL);  /* F54_ANALOG_CTRL99 (Integration Duration) */
	...
	SYNA_WRITE(dev, ANALOG_CONTROL2_REG, original_analog_ctrl2,
			sizeof(original_analog_ctrl2), ret, FAIL); /* F54_ANALOG_CTRL91 (Capacitance) */

	SYNA_WRITE(dev, CBC_TIMING_CONTROL_REG, original_cbc_timing_ctrl,
			sizeof(original_cbc_timing_ctrl), ret, FAIL); /* F54_ANALOG_CTRL182 (CBC TIMING CTRL) */

	SYNA_WRITE(dev, WAVEFORM_DURATION_CTRL_REG, original_intdur,
			sizeof(original_intdur), ret, FAIL);  /* F54_ANALOG_CTRL99 (Integration Duration) */
...
static int td4310_Test_Result_Data(struct device *dev, int test_type, int* test_result)
{
	...
	int boot_mode = touch_check_boot_mode(dev);
