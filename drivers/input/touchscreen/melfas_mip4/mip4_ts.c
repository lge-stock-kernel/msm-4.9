...
int mip4_ts_startup(struct mip4_ts_info *info)
{
	...
	TOUCH_D(BASE_INFO, "startup : reinit_completion\n");
#else
	init_completion(&info->startup_done);
	TOUCH_D(BASE_INFO, "startup : init_completion\n");
#endif
	...
	TOUCH_D(BASE_INFO, "startup : wait_for_completion_interruptible_timeout[%d]\n", STARTUP_TIMEOUT);
	ret = wait_for_completion_interruptible_timeout(&info->startup_done, msecs_to_jiffies(STARTUP_TIMEOUT));
	if (ret > 0) {
		TOUCH_D(BASE_INFO, "startup : completed[%d]\n", jiffies_to_msecs(ret));
	} else {
		TOUCH_D(BASE_INFO, "startup : timeout[%d]\n", ret);

#if USE_FW_RECOVERY
		/* Restore firmware */
		TOUCH_D(BASE_INFO, "flash_fail_type : critical\n");
		ret = mip4_ts_fw_restore_critical_section(info);
		if (ret < 0) {
			TOUCH_E("[ERROR] mip4_ts_fw_restore_critical_section\n");
		}
...
int mip4_ts_i2c_read(struct mip4_ts_info *info, char *write_buf, unsigned int write_len, char *read_buf, unsigned int read_len)
{
	...
	while (retry--) {
		...
		if (res == ARRAY_SIZE(msg)) {
			...
			TOUCH_E("[ERROR] i2c_transfer - errno[%d]\n", res);
		} else {
			TOUCH_E("[ERROR] i2c_transfer - size[%zu] result[%d]\n", ARRAY_SIZE(msg), res);
		}
	}
...
int mip4_ts_i2c_write(struct mip4_ts_info *info, char *write_buf, unsigned int write_len)
{
	...
	while (retry--) {
		...
		if (res == write_len) {
			...
			TOUCH_E("[ERROR] i2c_master_send - errno [%d]\n", res);
		} else {
			TOUCH_E("[ERROR] i2c_master_send - write[%d] result[%d]\n", write_len, res);
		}
	}
...
int mip4_ts_enable(struct mip4_ts_info *info)
{
	...
	if (info->enabled) {
		TOUCH_E("[ERROR] device already enabled\n");
		goto exit;
	}
	...
	if (wake_lock_active(&info->wake_lock)) {
		...
		TOUCH_D(BASE_INFO, "wake_unlock\n");
	}

	info->gesture_wakeup_mode = false;
	TOUCH_D(BASE_INFO, "gesture wake-up mode : off\n");
#else
#if USE_STARTUP_WAITING
	mip4_ts_startup(info);
#else
	...
	if (info->display_status && !info->irq_enabled) {
		TOUCH_I("Interrupt : Enable\n");
		enable_irq(info->client->irq);
		info->irq_enabled = true;
	} else {
		TOUCH_I("Skip interrupt enable, display_status[%d], irq_enabled[%d]\n", info->display_status, info->irq_enabled);
	}
#endif /* USE_STARTUP_WAITING */
...
exit:
	TOUCH_I(MIP4_TS_DEVICE_NAME" - Enabled\n");
...
int mip4_ts_disable(struct mip4_ts_info *info)
{
	...
	if (!info->enabled) {
		TOUCH_E("[ERROR] device already disabled\n");
		goto exit;
	}
	...
	TOUCH_D(BASE_INFO, "gesture wake-up mode : on\n");

	if (!wake_lock_active(&info->wake_lock)) {
		wake_lock(&info->wake_lock);
		TOUCH_D(BASE_INFO, "wake_lock\n");
	}
#else
	if (info->irq_enabled) {
		TOUCH_I("Interrupt : Disable\n");
		disable_irq(info->client->irq);
		info->irq_enabled = false;
	} else {
		TOUCH_I("Skip interrupt disable, display_status[%d], irq_enabled[%d]\n", info->display_status, info->irq_enabled);
	}
...
exit:
	TOUCH_I(MIP4_TS_DEVICE_NAME" - Disabled\n");
...
//	unsigned int blank;
	...
	if (event != FB_EVENT_BLANK) {
		TOUCH_D(BASE_INFO, "skip event [0x%02lX]\n", event);
		goto exit;
	}
#endif
...
//	TOUCH_D(BASE_INFO, "blank[0x%02X]\n", blank);
	TOUCH_D(BASE_INFO, "notify event [0x%02lX]\n", event);

//	switch (blank) {
	switch (event) {
	case FB_BLANK_UNBLANK:
	    /* Enable */
		TOUCH_D(BASE_INFO, "FB_BLANK_UNBLANK\n");
		TOUCH_I("Display : On\n");
		//mip4_ts_resume(&info->client->dev);
	...
	case FB_BLANK_POWERDOWN:
		/* Disable */
		TOUCH_D(BASE_INFO, "FB_BLANK_POWERDOWN\n");
		TOUCH_I("Display : Off\n");
		//mip4_ts_suspend(&info->client->dev);
		mutex_lock(&info->lock);
	...
	default:
//		TOUCH_D(BASE_INFO, "skip blank [0x%02X]\n", blank);
		break;
	}
...
int mip4_ts_get_ready_status(struct mip4_ts_info *info)
{
	...
	if (mip4_ts_i2c_read(info, wbuf, 2, rbuf, 1)) {
		TOUCH_E("[ERROR] mip4_ts_i2c_read\n");
		goto error;
	}
	ret = rbuf[0];
	...
	if ((ret == MIP4_CTRL_STATUS_NONE) || (ret == MIP4_CTRL_STATUS_LOG) || (ret == MIP4_CTRL_STATUS_READY)) {
		//TOUCH_D(BASE_INFO, "status[0x%02X]\n", ret);
	} else {
...
int mip4_ts_get_fw_version_from_bin(struct mip4_ts_info *info, u8 *ver_buf)
{
	...
	if (!fw) {
		TOUCH_E("[ERROR] request_firmware\n");
		goto error;
	}
	...
	if (mip4_ts_bin_fw_version(info, fw->data, fw->size, ver_buf)) {
		...
		TOUCH_E("[ERROR] mip4_ts_bin_fw_version\n");
		goto error;
	}
...
int mip4_ts_disable_esd_alert(struct mip4_ts_info *info)
{
	...
	if (rbuf[0] != 1) {
		TOUCH_D(BASE_INFO, "[ERROR] failed\n");
		goto error;
	}
...
static int mip4_ts_alert_handler_esd(struct mip4_ts_info *info, u8 *rbuf)
{
	...
	TOUCH_D(BASE_INFO, "frame_cnt[%d]\n", frame_cnt);

	if (frame_cnt == 0) {
		/* Panel defect, not ESD */
		info->esd_cnt++;
		TOUCH_D(BASE_INFO, "esd_cnt[%d]\n", info->esd_cnt);
...
static int mip4_ts_alert_handler_inputtype(struct mip4_ts_info *info, u8 *rbuf)
{
	...
	switch (input_type) {
	case 0:
		TOUCH_D(BASE_INFO, "Input type : Finger\n");
		break;
	case 1:
		TOUCH_D(BASE_INFO, "Input type : Glove\n");
		break;
	default:
		TOUCH_E("- Input type : Unknown[%d]\n", input_type);
		goto error;
	}
...
static int mip4_ts_alert_handler_image(struct mip4_ts_info *info, u8 *rbuf)
{
	...
	case MIP4_IMG_TYPE_GESTURE:
		if (mip4_ts_gesture_wakeup_event_handler(info, 1)) {
			TOUCH_E("[ERROR] mip4_ts_wakeup_event_handler\n");
			goto error;
		}
		if (mip4_ts_get_image(info, rbuf[1])) {
			TOUCH_E("[ERROR] mip4_ts_get_image\n");
			goto error;
		}
		break;
#endif /* USE_WAKEUP_GESTURE */
...
static irqreturn_t mip4_ts_interrupt(int irq, void *dev_id)
{
	...
	TOUCH_D(TRACE, "packet info : size[%d] category[%d]\n", size, category);
	...
	if (mip4_ts_i2c_read(info, wbuf, 2, rbuf, size)) {
		TOUCH_E("[ERROR] Read packet data\n");
		if (!startup) {
			info->i2c_error_cnt++;
			goto error;
		} else {
	...
	if (startup == false) {
		if (category == 0) {
			...
			/* Alert event */
			alert_type = rbuf[0];
			TOUCH_D(BASE_INFO, "alert type[%d]\n", alert_type);
			...
			case MIP4_ALERT_F1:
				...
				break;
			default:
				TOUCH_E("[ERROR] Unknown alert type[%d]\n", alert_type);
				goto error;
			}
		}
	} else {
		...
		if (category == 1) {
			...
			TOUCH_E("[ERROR] Unknown catetory[%d]\n", category);
			flash_fail_type = flash_fail_critical;
		}
		goto startup_exit;
#endif /* USE_STARTUP_WAITING */
...
startup_exit:
	...
	TOUCH_D(TRACE, "startup : complete_all\n");
...
int mip4_ts_config(struct mip4_ts_info *info)
{
	...
	TOUCH_D(BASE_INFO, "product_name[%s]\n", info->product_name);
	...
	TOUCH_I("F/W Version : %02X.%02X %02X.%02X %02X.%02X %02X.%02X\n", info->fw_version[0], info->fw_version[1], info->fw_version[2], info->fw_version[3], info->fw_version[4], info->fw_version[5], info->fw_version[6], info->fw_version[7]);
	...
	info->max_x = DISPLAY_MAX_X;
	info->max_y = DISPLAY_MAX_Y;
#else
	info->max_x = (rbuf[0]) | (rbuf[1] << 8);
	info->max_y = (rbuf[2]) | (rbuf[3] << 8);
#endif
	TOUCH_D(BASE_INFO, "max_x[%d] max_y[%d]\n", info->max_x, info->max_y);

	info->ppm_x = rbuf[12];
	info->ppm_y = rbuf[13];
	TOUCH_D(BASE_INFO, "ppm_x[%d] ppm_y[%d]\n", info->ppm_x, info->ppm_y);

	/* Node info */
	info->node_x = rbuf[4];
	info->node_y = rbuf[5];
	info->node_key = rbuf[6];
	TOUCH_D(BASE_INFO, "node_x[%d] node_y[%d] node_key[%d]\n", info->node_x, info->node_y, info->node_key);
	...
	while (retry--) {
		if (mip4_ts_i2c_read(info, wbuf, 2, rbuf, 3)) {
			TOUCH_E("[ERROR] mip4_ts_i2c_read - event format\n");
		} else {
			info->event_format = rbuf[0] | (rbuf[1] << 8);
			info->event_size = rbuf[2];
			if (info->event_size <= 0) {
				TOUCH_E("[ERROR] event_size[%d]\n", info->event_size);
				goto error;
			}
			TOUCH_D(BASE_INFO, "event_format[%d] event_size[%d]\n", info->event_format, info->event_size);
			break;
		}
	}
	if (retry < 0) {
		TOUCH_E("[ERROR] event format - retry limit\n");
		goto error;
	}
...
static int mip4_ts_config_platform(struct mip4_ts_info *info)
{
	...
	if (ACPI_SUCCESS(a_status)) {
		if (strncmp(dev_name(&a_dev->dev), ACPI_ID, 8) != 0) {
			TOUCH_E("[ERROR] ACPI_ID mismatch [%s]\n", dev_name(&a_dev->dev));
			ret = -EINVAL;
			goto exit;
		}
	}
	...
	if (&info->client->dev.of_node) {
		TOUCH_D(BASE_INFO, "Devicetree\n");

		ret = mip4_ts_parse_devicetree(&info->client->dev, info);
		if (ret) {
			TOUCH_E("[ERROR] mip4_ts_parse_devicetree\n");
			goto exit;
		}
	} else {
		TOUCH_E("[ERROR] of_node\n");
		goto exit;
	}
#else
	/* Platform device */
	TOUCH_D(BASE_INFO, "Platform device\n");

	info->pdata = dev_get_platdata(&info->client->dev);
	if (info->pdata == NULL) {
		ret = -EINVAL;
		TOUCH_E("[ERROR] dev_get_platdata\n");
		goto exit;
	}
...
int mip4_ts_fw_restore_critical_section(struct mip4_ts_info *info)
{
	...
	ret = mip4_ts_fw_restore_from_kernel(info, sections, retry);
	if (ret < 0) {
		TOUCH_E("[ERROR] mms_fw_restore_from_kernel\n");
		goto error;
	}
...
int mip4_ts_fw_restore_from_kernel(struct mip4_ts_info *info, u8 *sections, int retry)
{
	...
	if (retry <= 0) {
		TOUCH_E("[ERROR] mip4_ts_restore_fw failed\n");
		ret = fw_err_download;
	} else {
		ret = fw_err_none;
	}
...
int mip4_ts_fw_update_from_kernel(struct mip4_ts_info *info)
{
...
#if 0
	do {
		ret = mip4_ts_flash_fw(info, fw->data, fw->size, false, true, false);
		if (ret >= fw_err_none) {
			break;
		}
	} while (--retry);
	...
	if (!retry) {
		TOUCH_E("[ERROR] mip4_ts_flash_fw failed\n");
		ret = fw_err_download;
	}
#else
	ret = mip4_ts_flash_fw(info, fw->data, fw->size, false, true, false);
 ...
 */
int mip4_ts_fw_update_from_storage(struct mip4_ts_info *info, char *path, bool force, bool isc)
{
	struct file *fp;
	mm_segment_t old_fs;
	...
	if (IS_ERR(fp)) {
		TOUCH_E("[ERROR] file_open - path[%s]\n", path);
		ret = fw_err_file_open;
		goto error;
	}
	...
	if (fw_size > 0) {
		...
		TOUCH_D(BASE_INFO, "path[%s] size[%zu]\n", path, fw_size);

		if (nread != fw_size) {
			TOUCH_E("[ERROR] vfs_read - size[%zu] read[%zu]\n", fw_size, nread);
			ret = fw_err_file_read;
		} else {
			/* Update firmware */
			ret = mip4_ts_flash_fw(info, fw_data, fw_size, force, true, isc);
		}

		kfree(fw_data);
	} else {
		TOUCH_E("[ERROR] fw_size[%zu]\n", fw_size);
		ret = fw_err_file_read;
	}
...
static int mip4_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (!i2c_check_functionality(adapter, I2C_FUNC_I2C)) {
		TOUCH_E("[ERROR] i2c_check_functionality\n");
		ret = -EIO;
		goto error_i2c;
	}
	...
	info->bl_path_ext = kstrdup(BL_PATH_EXTERNAL, GFP_KERNEL);
	...
	if (ret) {
		TOUCH_E("[ERROR] mip4_ts_config_platform\n");
		goto error_info;
	}
	...
	ret = mip4_ts_fw_update_from_kernel(info);
	if (ret) {
		TOUCH_E("[ERROR] mip4_ts_fw_update_from_kernel\n");
	}
	...
	if (ret) {
		TOUCH_E("[ERROR] request_threaded_irq\n");
		goto error_irq;
	}
	...
	if (ret) {
		TOUCH_E("[ERROR] input_register_device\n");
		ret = -EIO;
		goto error_device;
	}
	...
	mip4_fb_register_client(&info->fb_notifier);
#endif /* USE_FB_NOTIFY */

#if USE_DEV
	/* Create dev node (optional) */
	if (mip4_ts_dev_create(info)) {
		TOUCH_E("[ERROR] mip4_ts_dev_create\n");
	}
	...
	if (mip4_ts_sysfs_create(info)) {
		TOUCH_E("[ERROR] mip4_ts_sysfs_create\n");
	}
#endif /* USE_SYS */
	...
	if (mip4_ts_sysfs_cmd_create(info)) {
		TOUCH_E("[ERROR] mip4_ts_sysfs_cmd_create\n");
	}
#endif /* USE_CMD */

	/* Create sysfs */
	if (sysfs_create_group(&client->dev.kobj, &mip4_ts_attr_group)) {
		TOUCH_E("[ERROR] sysfs_create_group\n");
	}
	if (sysfs_create_link(NULL, &client->dev.kobj, MIP4_TS_DEVICE_NAME)) {
		TOUCH_E("[ERROR] sysfs_create_link\n");
	}

	TOUCH_D(BASE_INFO, "[DONE]\n");
	TOUCH_I("MELFAS " CHIP_NAME " Touchscreen\n");
	return 0;
...
error_info:
error_i2c:
	TOUCH_D(BASE_INFO, "[ERROR]\n");
	TOUCH_E("MELFAS " CHIP_NAME " Touchscreen initialization failed.\n");
	return ret;
}
...
static int mip4_ts_remove(struct i2c_client *client)
{
	...
	mip4_fb_unregister_client(&info->fb_notifier);
#endif /* USE_FB_NOTIFY */
