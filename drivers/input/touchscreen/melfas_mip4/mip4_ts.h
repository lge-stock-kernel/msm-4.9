...
#define DD14_RESOLUTION					1
#define WIDEPD_RESOLUTION				1
#define SMALLPD_RESOLUTION				1
...
#define TOUCH_DEBUG_PRINT_MASK	BASE_INFO;

#if defined(TOUCH_INFO_PRINT)
#define TOUCH_I(fmt, args...) 	printk(KERN_ERR "[Touch] [%s %d] " fmt, __FUNCTION__, __LINE__, ##args)
#else
#define TOUCH_I(fmt, args...)    do {} while(0)
#endif
...
#define TOUCH_D(condition, fmt, args...)												\
	do {																				\
		if (unlikely(touch_debug_mask & (condition)))									\
			printk(KERN_ERR "[Touch D] [%s %d] " fmt, __FUNCTION__, __LINE__, ##args);	\
	} while (0)
#else
#define TOUCH_D(fmt, args...)    do {} while(0)
#endif

#if defined(TOUCH_ERROR_PRINT)
#define TOUCH_E(fmt, args...) printk(KERN_ERR "[Touch E] [%s %d] " fmt, __FUNCTION__, __LINE__, ##args)
#else
#define TOUCH_E(fmt, args...)    do {} while(0)
#endif
...
#define USE_FB_NOTIFY         1 /* 0 (default) or 1 */
...
/* Firmware update */
#define FW_PATH_INTERNAL "melfas/melfas_mip4_ts.img"  /* path of firmware included in the kernel image (/firmware) */
#define FW_PATH_EXTERNAL "/sdcard/melfas_mip4_ts.fw" /* path of firmware in the external storage */
#define BL_PATH_EXTERNAL "/sdcard/melfas_mip4_ts.bl" /* path of bootloader in the external storage */
...
#define DISPLAY_MAX_X			1920
#define DISPLAY_MAX_Y			1080

#define X_AXIS					468750
#define Y_AXIS					263672
#define AXIS_RATIO				1000000
#endif
...
int mip4_ts_aot_config(struct mip4_ts_info *info, u8 aod, u8 sp, u8 double_tap, u16 width, u16 height, u16 offset_x, u16 offset_y);
#endif
