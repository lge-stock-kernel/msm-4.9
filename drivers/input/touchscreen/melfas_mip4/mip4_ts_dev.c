...
static ssize_t mip4_ts_dev_fs_read(struct file *fp, char *rbuf, size_t cnt, loff_t *fpos)
{
	...
	//TOUCH_D(BASE_INFO, "copy_to_user[%d]\n", ret);
...
int mip4_ts_dev_create(struct mip4_ts_info *info)
{
	...
	if (alloc_chrdev_region(&info->mip4_ts_dev, 0, 1, MIP4_TS_DEVICE_NAME)) {
		TOUCH_E("[ERROR] alloc_chrdev_region\n");
		ret = -ENOMEM;
		goto error;
	}
		...
		for (i_col = 0; i_col < col_num; i_col++) {
			if (data_type_sign == 0) {
				...
				case 4:
					...
					break;
				default:
					TOUCH_E("[ERROR] data_type_size[%d]\n", data_type_size);
					goto error;
				}
				value = (int)uValue;
			} else {
...
static int mip4_ts_proc_vector_data(struct mip4_ts_info *info, u8 data_type_size, u8 data_type_sign, u8 buf_addr_h, u8 buf_addr_l, u8 key_num, u8 vector_num, u16 *vector_id, u16 *vector_elem_num, int table_size)
{
	...
	for (i = 0; i < vector_num; i++) {
		TOUCH_D(BASE_INFO, "vector_elem_num(%d)[%d]\n", i, vector_elem_num[i]);
	}
	TOUCH_D(BASE_INFO, "key_num[%d] total_len[%d]\n", key_num, total_len);
...
int mip4_ts_run_test(struct mip4_ts_info *info, u8 test_type)
{
	...
	TOUCH_D(BASE_INFO, "set control mode\n");
	...
	wbuf[2] = test_type;
	if (mip4_ts_i2c_write(info, wbuf, 3)) {
		TOUCH_E("[ERROR] Write test type\n");
		goto error;
	}

	TOUCH_D(BASE_INFO, "set test type\n");
	...
	TOUCH_D(BASE_INFO, "row_num[%d] col_num[%d] buffer_col_num[%d] rotate[%d] key_num[%d]\n", row_num, col_num, buffer_col_num, rotate, key_num);
	TOUCH_D(BASE_INFO, "data_type[0x%02X] data_type_sign[%d] data_type_size[%d]\n", data_type, data_type_sign, data_type_size);
	TOUCH_D(BASE_INFO, "vector_num[%d]\n", vector_num);

	if (vector_num > 0) {
		wbuf[0] = MIP4_R0_TEST;
		wbuf[1] = MIP4_R1_TEST_VECTOR_INFO;
		if (mip4_ts_i2c_read(info, wbuf, 2, rbuf, (vector_num * 4))) {
			TOUCH_E("[ERROR] Read vector info\n");
			goto error;
		}
		for (i = 0; i < vector_num; i++) {
			vector_id[i] = rbuf[i * 4 + 0] | (rbuf[i * 4 + 1] << 8);
			vector_elem_num[i] = rbuf[i * 4 + 2] | (rbuf[i * 4 + 3] << 8);
			TOUCH_D(BASE_INFO, "vector[%d] : id[%d] elem_num[%d]\n", i, vector_id[i], vector_elem_num[i]);
		}
	}
	...
	buf_addr_h = rbuf[1];
	buf_addr = (buf_addr_h << 8) | buf_addr_l;
	TOUCH_D(BASE_INFO, "table buf_addr[0x%02X 0x%02X][0x%04X]\n", buf_addr_h, buf_addr_l, buf_addr);
	...
	if (table_size > 0) {
		if (mip4_ts_proc_table_data(info, data_type_size, data_type_sign, buf_addr_h, buf_addr_l, row_num, col_num, buffer_col_num, rotate)) {
			TOUCH_E("[ERROR] mip4_ts_proc_table_data\n");
			goto error;
		}
	}
	if ((key_num > 0) || (vector_num > 0)) {
		...
		TOUCH_D(BASE_INFO, "vector buf_addr[0x%02X 0x%02X][0x%04X]\n", buf_addr_h, buf_addr_l, buf_addr);
		...
		if (mip4_ts_proc_vector_data(info, data_type_size, data_type_sign, buf_addr_h, buf_addr_l, key_num, vector_num, vector_id, vector_elem_num, table_size)) {
			TOUCH_E("[ERROR] mip4_ts_proc_vector_data\n");
			goto error;
		}
	}
	...
	TOUCH_D(BASE_INFO, "set normal mode\n");
...
int mip4_ts_get_image(struct mip4_ts_info *info, u8 image_type)
{
	...
	TOUCH_D(BASE_INFO, "[START]\n");
	TOUCH_D(BASE_INFO, "image_type[%d]\n", image_type);

	while (busy_cnt--) {
		if (info->test_busy == false) {
			break;
		}
		TOUCH_D(BASE_INFO, "busy_cnt[%d]\n", busy_cnt);
	...
	case MIP4_IMG_TYPE_INTENSITY:
		TOUCH_D(BASE_INFO, "=== Intensity Image ===\n");
		snprintf(info->print_buf, PAGE_SIZE, "\n=== Intensity Image ===\n\n");
		break;
	case MIP4_IMG_TYPE_RAWDATA:
		TOUCH_D(BASE_INFO, "=== Rawdata Image ===\n");
		snprintf(info->print_buf, PAGE_SIZE, "\n=== Rawdata Image ===\n\n");
		break;
	case MIP4_IMG_TYPE_GESTURE:
		TOUCH_D(BASE_INFO, "=== Gesture Image ===\n");
		snprintf(info->print_buf, PAGE_SIZE, "\n=== Gesture Image ===\n\n");
		break;
	case MIP4_IMG_TYPE_HSELF_RAWDATA:
		TOUCH_D(BASE_INFO, "=== Self Rawdata Image ===\n");
		snprintf(info->print_buf, PAGE_SIZE, "\n=== Self Rawdata Image ===\n\n");
		break;
	case MIP4_IMG_TYPE_HSELF_INTENSITY:
		TOUCH_D(BASE_INFO, "=== Self Intensity Image ===\n");
	...
	TOUCH_D(BASE_INFO, "set image type\n");
...
static ssize_t mip4_ts_sys_fw_version(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (mip4_ts_get_fw_version(info, rbuf)) {
		TOUCH_E("[ERROR] mip4_ts_get_fw_version\n");
...
static ssize_t mip4_ts_sys_fw_update_from_storage(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	/* Update firmware */
	ret = mip4_ts_fw_update_from_storage(info, info->fw_path_ext, true, false);
...
/*
* Update bootloader from external storage
*/
static ssize_t mip4_ts_sys_bl_update_from_storage(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct mip4_ts_info *info = i2c_get_clientdata(client);
	int result = 0;
	...
	/* Update firmware */
	ret = mip4_ts_fw_update_from_storage(info, info->bl_path_ext, true, true);

	switch (ret) {
	case fw_err_none:
		snprintf(data, sizeof(data), "Bootloader update success.\n");
		break;
	case fw_err_uptodate:
		snprintf(data, sizeof(data), "Bootloader is already up-to-date.\n");
		break;
	case fw_err_download:
		snprintf(data, sizeof(data), "Bootloader update failed : Download error\n");
		break;
	case fw_err_file_type:
		snprintf(data, sizeof(data), "Bootloader update failed : File type error\n");
		break;
	case fw_err_file_open:
		snprintf(data, sizeof(data), "Bootloader update failed : File open error[%s]\n", info->bl_path_ext);
		break;
	case fw_err_file_read:
		snprintf(data, sizeof(data), "Bootloader update failed : File read error\n");
		break;
	default:
		snprintf(data, sizeof(data), "Bootloader update failed.\n");
		break;
	}
...
static ssize_t mip4_ts_sys_bin_version(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (mip4_ts_get_fw_version_from_bin(info, rbuf)) {
		TOUCH_E("[ERROR] mip4_ts_get_fw_version_from_bin\n");
		snprintf(data, sizeof(data), "BIN Version : ERROR\n");
		goto error;
	}

	TOUCH_I("BIN Version : %02X.%02X/%02X.%02X/%02X.%02X/%02X.%02X\n", rbuf[0], rbuf[1], rbuf[2], rbuf[3], rbuf[4], rbuf[5], rbuf[6], rbuf[7]);
...
static ssize_t mip4_ts_sys_flash_cal_table(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (mip4_ts_flash_cal_table(info)) {
		TOUCH_E("[ERROR] mip4_ts_flash_cal_table\n");
...
static ssize_t mip4_ts_sys_mode_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (mip4_ts_i2c_write(info, wbuf, 3)) {
		...
		TOUCH_I("addr[0x%02X%02X] value[%d]\n", wbuf[0], wbuf[1], value);
	}
...
static ssize_t mip4_ts_sys_test(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (!strcmp(attr->attr.name, "test_cm")) {
		...
		TOUCH_E("[ERROR] Unknown test[%s]\n", attr->attr.name);
		ret = snprintf(buf, PAGE_SIZE, "%s\n", "ERROR : Unknown test type");
		goto error;
	}

	if (mip4_ts_run_test(info, test_type)) {
		TOUCH_E("[ERROR] mip4_ts_run_test\n");
		ret = snprintf(buf, PAGE_SIZE, "%s\n", "ERROR");
		goto error;
	}
...
static DEVICE_ATTR(bl_update_storage, S_IRUGO, mip4_ts_sys_bl_update_from_storage, NULL);
...
static struct attribute *mip4_ts_sys_attr[] = {
	...
	&dev_attr_bl_update_storage.attr,
