...
static int ili9881h_notify(struct device *dev, ulong event, void *data)
{
	...
	if (boot_mode == TOUCH_CHARGER_MODE	|| boot_mode == TOUCH_LAF_MODE || boot_mode == TOUCH_RECOVERY_MODE){
...
static int ili9881h_upgrade(struct device *dev)
{
	...
	if ((boot_mode == TOUCH_LAF_MODE) || (boot_mode == TOUCH_CHARGER_MODE) || (boot_mode == TOUCH_RECOVERY_MODE)) {
