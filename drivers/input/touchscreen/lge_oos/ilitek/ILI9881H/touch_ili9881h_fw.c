...
#endif
		TOUCH_I("New firmware version is same as before, goto check_flash_crc!!\n");
...
int ili9881h_fw_upgrade(struct device *dev, const struct firmware *fw)
{
	...
	if (ret < 0 || retry <=0) {
		TOUCH_E("Still upgrade failed after retried 3 times, erasing all fw data !\n");
		ili9881h_ice_mode_enable(dev, MCU_STOP);
		touch_msleep(25);
		ili9881h_flash_erase_sector(dev);
