...
int ft8006p_change_op_mode(struct device *dev, u8 op_mode)
{
	...
	data = 0x00;
	ret = ft8006p_reg_read(dev, 0x00, &data, 1);
	if(ret < 0) {
		TOUCH_E("i2c error\n");
		return ret;
	}
	...
	data = op_mode;
	ret = ft8006p_reg_write(dev, 0x00, &data, 1);
	if(ret < 0) {
		TOUCH_E("i2c error\n");
		return ret;
	}
...
//	mdelay(300);
	...
	if((op_mode == FTS_FACTORY_MODE) && (d->state == TC_STATE_LPWG)) {
		...
		data = 0x01;
		ret = ft8006p_reg_write(dev, 0x88, &data, 1);
		if(ret < 0) {
			TOUCH_E("i2c error\n");
			return ret;
		}
...
int ft8006p_switch_cal(struct device *dev, u8 cal_en)
{
#if 0
	int i = 0;
	u8 ret;
	...
	data = 0x00;
	ret = ft8006p_reg_read(dev, 0xEE, &data, 1);
	if(ret < 0) {
		TOUCH_E("i2c error\n");
		return ret;
	}
	...
	data = cal_en;
	ret = ft8006p_reg_write(dev, 0xEE, &data, 1);
	if(ret < 0) {
		TOUCH_E("i2c error\n");
		return ret;
	}
...
int ft8006p_prd_check_ch_num(struct device *dev)
{
	u8 ret;
	u8 data;
	...
	ret = ft8006p_reg_read(dev, 0x02, &data, 1);
	if(ret < 0) {
		TOUCH_E("i2c error\n");
		return ret;
	}
	...
	ret = ft8006p_reg_read(dev, 0x03, &data, 1);
	if(ret < 0) {
		TOUCH_E("i2c error\n");
		return ret;
	}
...
int ft8006p_prd_get_raw_data(struct device *dev)
{
	int i, j, k;
	int ret = 0;
	u8 data = 0x00;
	...
	data = 0x00;
	ret = ft8006p_reg_write(dev, 0x06, &data, 1);
	if (ret < 0) {
		TOUCH_E("i2c error\n");
		return ret;
	}
	...
	for (k = 0; k < 3; k++)
	{
		...
		if(bscan==true){
			/* Read Raw data */
			rawdata = 0xAD;
			ret = ft8006p_reg_write(dev, 0x01, &rawdata, 1);
			if(ret < 0) {
				TOUCH_E("i2c error\n");
				return ret;
			}
			...
			ret = ft8006p_reg_read(dev, 0x6A, &i2c_data[0], MAX_COL*MAX_ROW*2);
			if(ret < 0) {
				TOUCH_E("i2c error\n");
				return ret;
			}
...
int ft8006p_prd_get_noise_data(struct device *dev)
{
	int i, j, k;
	int ret = 0;
	u8 data = 0x00;
//	u8 frame_count;
	...
	//low  byte
	data = 0x32;
	ret = ft8006p_reg_write(dev, 0x12, &data, 1);
	if (ret < 0) {
		TOUCH_E("i2c error\n");
		return ret;
	}
	...
	//high  byte
	data = 0x00;
	ret = ft8006p_reg_write(dev, 0x13, &data, 1);
	if (ret < 0) {
		TOUCH_E("i2c error\n");
		return ret;
	}
	...
	// Start Noise Test
	//data = 0x00; //FT8006M: 0x00;
	data = 0x01; //FT8006P: 0x01;
	ret = ft8006p_reg_write(dev, 0x11, &data, 1);
	if (ret < 0) {
		TOUCH_E("i2c error\n");
		return ret;
	}
	...
	// Clear Raw Data Addr
	data = 0xAD;
	ret = ft8006p_reg_write(dev, 0x01, &data, 1);
	if (ret < 0) {
		TOUCH_E("i2c error\n");
		return ret;
	}
	...
	for (i = 0; i < 100; i++)
	{
		ret = ft8006p_reg_read(dev, 0x11, &data, 1);
		if (ret < 0) {
			TOUCH_E("i2c error\n");
			return ret;
		}
	...
	ret = ft8006p_reg_read(dev, 0x13, &data, 1);
	if(ret < 0) {
		TOUCH_E("i2c error\n");
		return ret;
	}
...
int ft8006p_prd_get_jitter_data(struct device *dev)
{
	int i, j, k;
	int ret = 0;
	u8 data = 0x00;
...
int ft8006p_prd_get_ib_data(struct device *dev)
{
	int i, j;
	int ret = 0;
	u8 data = 0x00;
	...
	for (i = 0; (total - TEST_PACKET_LENGTH*i) > 0; i++)
	{
		...
		ret = ft8006p_reg_write(dev, 0x18, &data, 1);
		if (ret < 0) {
			TOUCH_E("i2c error\n");
			return ret;
		}
		...
		data = (offset & 0x00FF);
		ret = ft8006p_reg_write(dev, 0x19, &data, 1);
		if (ret < 0) {
			TOUCH_E("i2c error\n");
			return ret;
		}
		...
		ret = ft8006p_reg_read(dev, 0x6E, &i2c_data[offset], read_len);
		if (ret < 0) {
			TOUCH_E("i2c error\n");
			return ret;
		}
...
int ft8006p_prd_get_delta_data(struct device *dev)
{
	int i, j, k;
	int ret = 0;
	u8 data = 0x00;
	...
	for (i = 0; (total - TEST_PACKET_LENGTH*i) > 0; i++)
	{
		...
		data = (offset & 0xFF00) >> 8;
		ret = ft8006p_reg_write(dev, 0x1C, &data, 1);
		if (ret < 0) {
			TOUCH_E("i2c error\n");
			return ret;
		}
		...
		data = (offset & 0x00FF);
		ret = ft8006p_reg_write(dev, 0x1D, &data, 1);
		if (ret < 0) {
			TOUCH_E("i2c error\n");
			return ret;
		}
		...
		ret = ft8006p_reg_read(dev, 0x6A, &i2c_data[offset], read_len);
		if (ret < 0) {
			TOUCH_E("i2c error\n");
			return ret;
		}
...
int ft8006p_prd_test_data(struct device *dev, int test_type, int* test_result)
{
	...
	*test_result = TEST_FAIL;
	...
	ret = snprintf(log_buf, sizeof(log_buf), "IC F/W Version : V%d.%02d\n", d->ic_info.is_official, d->ic_info.fw_version);
	...
	switch (test_type) {
		case RAW_DATA_TEST:
			...
			break;
		case IB_DATA_TEST:
			ret += snprintf(log_buf + ret, sizeof(log_buf) - ret, "============= IB Data Test Result =============\n");
			limit_upper = IB_MAX;
			limit_lower = IB_MIN;
			break;
		case NOISE_TEST:
			ret += snprintf(log_buf + ret, sizeof(log_buf) - ret, "============= Noise Test Result =============\n");
		...
		case LPWG_IB_DATA_TEST:
			ret += snprintf(log_buf + ret, sizeof(log_buf) - ret, "============= LPWG IB Data Test Result =============\n");
			limit_upper = LPWG_IB_MAX;
			limit_lower = LPWG_IB_MIN;
			break;
		case LPWG_NOISE_TEST:
			ret += snprintf(log_buf + ret, sizeof(log_buf) - ret, "============= LPWG Noise Test Result =============\n");
	...
	for (i = 0; i < MAX_ROW; i++) {
		...
		if(d->ic_info.chip_id_low == 0x22) /* Modify due to reverse from top to bottom  */
			k = MAX_ROW-i-1;
		else
			k = i;
		for (j = 0; j < MAX_COL; j++) {
			...
			if (test_type == RAW_DATA_TEST || test_type == LPWG_RAW_DATA_TEST) {
				ret += snprintf(log_buf + ret, sizeof(log_buf) - ret, "%5d ", fts_data[k][j]);
			}
			else {
				ret += snprintf(log_buf + ret, sizeof(log_buf) - ret, "%4d ", fts_data[k][j]);
			}

			if (check_limit && (fts_data[k][j] < limit_lower || fts_data[k][j] > limit_upper)) {
				fail_count++;
			}
			if (fts_data[k][j] < min)
				min = fts_data[k][j];
...
static ssize_t show_sd(struct device *dev, char *buf)
{
	...
	// Change clb switch
	ret = ft8006p_switch_cal(dev, 1);
	if(ret < 0)
		goto FAIL;

	// Change to factory mode
	ret = ft8006p_change_op_mode(dev, FTS_FACTORY_MODE);
	if(ret < 0)
		goto FAIL;
	...
	ret = ft8006p_prd_check_ch_num(dev);
	if(ret < 0)
		goto FAIL;

	ret = ft8006p_prd_get_raw_data(dev);
	if(ret < 0)
		goto FAIL;

	ret_size = ft8006p_prd_test_data(dev, RAW_DATA_TEST, &test_result);
	...
	ret = ft8006p_prd_get_ib_data(dev);
	if(ret < 0)
		goto FAIL;

	ret_size = ft8006p_prd_test_data(dev, IB_DATA_TEST, &test_result);
	...
	msleep(30);
	if ((d->ic_info.chip_id_low == 0x06)|| (d->ic_info.chip_id_low == 0x22)) { //0x07 for FT86XX/FT87xx, 0x06 for FT80xx , 0x22 for FT8006P
		// Start to noise test
		...
		ret = ft8006p_prd_get_noise_data(dev);
		if(ret < 0)
			goto FAIL;

		ret_size = ft8006p_prd_test_data(dev, NOISE_TEST, &test_result);
	...
	ret = ft8006p_change_op_mode(dev, FTS_WORK_MODE);
	if(ret < 0)
		goto FAIL;
...
//	touch_interrupt_control(ts->dev, INTERRUPT_DISABLE);
	...
	ret = ft8006p_prd_get_delta_data(dev);
	if(ret < 0)
		goto FAIL;
	...
	ret_size = ft8006p_prd_test_data(dev, DELTA_SHOW, &test_result);
...
//	touch_interrupt_control(ts->dev, INTERRUPT_DISABLE);
	...
	ret = ft8006p_prd_get_jitter_data(dev);
	if(ret < 0)
		goto FAIL;
	...
	ret_size = ft8006p_prd_test_data(dev, JITTER_TEST, &test_result);
	memcpy(buf, log_buf, ret_size);
...
static ssize_t show_lpwg_sd(struct device *dev, char *buf)
{
	...
	ret_size = ft8006p_prd_test_data(dev, LPWG_RAW_DATA_TEST, &test_result);
	...
	ret_size = ft8006p_prd_test_data(dev, LPWG_IB_DATA_TEST, &test_result);
	...
	if ((d->ic_info.chip_id_low == 0x06)|| (d->ic_info.chip_id_low == 0x22)) { //0x07 for FT86XX/FT87xx, 0x06 for FT80xx , 0x22 for FT8006P
		...
		ret_size = ft8006p_prd_test_data(dev, LPWG_NOISE_TEST, &test_result);
	...
	char *buf, loff_t off, size_t count)
{
	...
	struct ft8006p_data *d = to_ft8006p_data(ts->dev);
	...
	if (off == 0) {
		if (d->prd_delta_data == NULL) {
			...
			// Change clb switch
			ret = ft8006p_switch_cal(ts->dev, 0);


			ret = ft8006p_change_op_mode(ts->dev, FTS_FACTORY_MODE);



			ret = ft8006p_prd_get_delta_data(ts->dev);


			ret = ft8006p_change_op_mode(ts->dev, FTS_WORK_MODE);


			TOUCH_I("Show Delta Data OK !!!\n");

			ret_size = ft8006p_prd_test_data(ts->dev, DELTA_SHOW, &test_result);
	...
	char *buf, loff_t off, size_t count)
{
	...
	if (off == 0) {
		if (d->prd_rawdata_data == NULL) {
			...
			ret = ft8006p_prd_get_raw_data(ts->dev);
			// Data Select to raw data
			data = 0x01;
			ret = ft8006p_reg_write(ts->dev, 0x06, &data, 1);

			ret = ft8006p_change_op_mode(ts->dev, FTS_WORK_MODE);

			TOUCH_I("Show Raw Data OK !!!\n");

			ret_size = ft8006p_prd_test_data(ts->dev, RAW_DATA_TEST, &test_result);
...
static int ft8006p_prd_attr_delta_init(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8006p_data *d = to_ft8006p_data(dev);
...
static int ft8006p_prd_attr_rawdata_init(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8006p_data *d = to_ft8006p_data(dev);
...
int ft8006p_prd_register_sysfs(struct device *dev)
{
	...
	ft8006p_prd_attr_delta_init(dev);
	ft8006p_prd_attr_rawdata_init(dev);
