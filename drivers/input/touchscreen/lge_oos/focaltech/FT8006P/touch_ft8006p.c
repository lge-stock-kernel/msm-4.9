...
// Definitions for Debugging Failure Reason in SWIPE
enum {
	SWIPE_DEBUG_DISABLE = 0,
	SWIPE_DEBUG_ALWAYS,
	SWIPE_DEBUG_BUFFER,
	SWIPE_DEBUG_BUFFER_ALWAYS,
};

#define SWIPE_FR_BUF_LEN	13
#define SWIPE_FR_NUM		12

static const char const *swipe_debug_str[SWIPE_FR_NUM + 1] = {
	"NONE",
	"ERROR",
	"FINGER_FAST_RELEASE",
	"MULTI_FINGER",
...
//	struct ft8006p_data *d = to_ft8006p_data(dev);
	...
	case SW_RESET:
		TOUCH_I("%s : SW Reset\n", __func__);
//		ft8006p_cmd_write(dev, CMD_ENA);
//		ft8006p_cmd_write(dev, CMD_RESET_LOW);
		touch_msleep(5);
//		ft8006p_cmd_write(dev, CMD_RESET_HIGH);
//		ft8006p_cmd_write(dev, CMD_DIS);
...
{
        struct touch_core_data *ts = to_touch_core(dev);
        int mm_to_point = 11; // 1 mm -> about X point
...
static int ft8006p_swipe_active_area(struct device *dev)
{
        struct touch_core_data *ts = to_touch_core(dev);
        u8 active_area[8] = {0, };
        u8 start_area[8] = {0, };
        ...
        /* Set by ft8006_get_swipe_info func */
        active_area[0] = ts->swipe.info.area.x1 >> 8;
        active_area[1] = ts->swipe.info.area.x1;
        active_area[2] = ts->swipe.info.area.y1 >> 8;
        active_area[3] = ts->swipe.info.area.y1;
        active_area[4] = ts->swipe.info.area.x2 >> 8;
        active_area[5] = ts->swipe.info.area.x2;
        active_area[6] = ts->swipe.info.area.y2 >> 8;
        active_area[7] = ts->swipe.info.area.y2;

        start_area[0] = ts->swipe.info.start.x1 >> 8;
        start_area[1] = ts->swipe.info.start.x1;
        start_area[2] = ts->swipe.info.start.y1 >> 8;
        start_area[3] = ts->swipe.info.start.y1;
        start_area[4] = ts->swipe.info.start.x2 >> 8;
        start_area[5] = ts->swipe.info.start.x2;
        start_area[6] = ts->swipe.info.start.y2 >> 8;
        start_area[7] = ts->swipe.info.start.y2;


        TOUCH_I("%s - active_area:  x1[%d], y1[%d], x2[%d], y2[%d]\n", __func__,
                        ((active_area[0] << 8) | active_area[1]), ((active_area[2] << 8) | active_area[3]),
                        ((active_area[4] << 8) | active_area[5]), ((active_area[6] << 8) | active_area[7]));
        TOUCH_I("%s - start_area:  x1[%d], y1[%d], x2[%d], y2[%d]\n", __func__,
                        ((start_area[0] << 8) | start_area[1]), ((start_area[2] << 8) | start_area[3]),
                        ((start_area[4] << 8) | start_area[5]), ((start_area[6] << 8) | start_area[7]));

#if 1 // After Activ area firmeare modify
        ret = ft8006p_reg_write(dev, SWIPE_ACTIVE_AREA_X1_H_CTRL, &active_area[0], sizeof(active_area));
#endif
        ret = ft8006p_reg_write(dev, SWIPE_START_AREA_X1_H_CTRL, &start_area[0], sizeof(start_area));

        return ret;
}

static int ft8006p_swipe_command(struct device *dev, int cmd)
{
        struct touch_core_data *ts = to_touch_core(dev);
        u8 swipe_data = 0;
        int ret = 0;
        ...
        switch (cmd) {
                case SWIPE_ENABLE_CMD:
                        //swipe_data  = ts->tci.mode | (ts->swipe.mode << 2);
                        swipe_data  = ts->tci.mode | (0x04);
                        ret = ft8006p_reg_write(dev, 0xD0, &swipe_data, sizeof(swipe_data));
                        break;
                case SWIPE_DISABLE_CMD:
                        //swipe_data = ts->tci.mode | (0 << 2);
                        swipe_data = ts->tci.mode | (0x00);
                        ret = ft8006p_reg_write(dev, 0xD0, &swipe_data, sizeof(swipe_data));
                        break;
                case SWIPE_DIST_CMD:
                        break;
                ...
                case SWIPE_ACTIVE_AREA_CMD:
                        ret = ft8006p_swipe_active_area(dev);
                        break;
...
static int ft8006p_swipe_control(struct device *dev)
{
        struct touch_core_data *ts = to_touch_core(dev);
	struct ft8006p_data *d = to_ft8006p_data(dev);
        u8 swipe_data = 0x0;
        ...
        if (ts->lpwg.screen) { // Perhaps the part that needs to change
                TOUCH_I("%s swipe mode = %s\n", __func__,  "Disable");
                ret = ft8006p_swipe_command(dev, SWIPE_DISABLE_CMD);
        } else {
                TOUCH_I("%s swipe mode = %s\n", __func__,  "Start");
		// swipe select
		swipe_init = 0x00; //swipe up
		ret = ft8006p_reg_write(dev, 0x60, &swipe_init, sizeof(swipe_init));

		// swipe debug enable
		swipe_debug = d->swipe_debug_type;
		ret = ft8006p_reg_write(dev, 0x95, &swipe_debug, sizeof(swipe_debug));

                swipe_data = ts->swipe.info.distance;
                ret = ft8006p_reg_write(dev, SWIPE_DIST_THRESHOLD_CTRL, &swipe_data, sizeof(swipe_data));

                swipe_data_2[0] = ts->swipe.info.ratio_thres;
                swipe_data_2[1] = ts->swipe.info.min_time >> 8;
                swipe_data_2[2] = ts->swipe.info.min_time;
                swipe_data_2[3] = ts->swipe.info.max_time >> 8;
                swipe_data_2[4] = ts->swipe.info.max_time;
                swipe_data_2[5] = ts->swipe.info.wrong_dir_thes;
                ret = ft8006p_reg_write(dev, SWIPE_RATIO_THRESHOLD_CTRL, &swipe_data_2[0], sizeof(swipe_data_2));

                ret = ft8006p_swipe_command(dev, SWIPE_ACTIVE_AREA_CMD);

                swipe_data = ts->swipe.info.init_rat_chk_dist;
                ret = ft8006p_reg_write(dev, SWIPE_INIT_RATIO_CHK_DIST_CTRL, &swipe_data, sizeof(swipe_data));

                swipe_data = ts->swipe.info.init_rat_thres;
                ret = ft8006p_reg_write(dev, SWIPE_INIT_RATIO_THD_CTRL, &swipe_data, sizeof(swipe_data));

		// swipe Enable
                ret = ft8006p_swipe_command(dev, SWIPE_ENABLE_CMD);
                TOUCH_I("%s swipe mode = %s\n", __func__,  "Enable");
...
static int ft8006p_tci_control(struct device *dev, int type)
{
	...
	case TCI_CTRL_CONFIG_COMMON:
		...
		data = 0x00;
		ret |= ft8006p_reg_write(dev, 0xB9, &data, 1);	// Active Area MSB of Y1 (34)
		data = 0x7E;
		ret |= ft8006p_reg_write(dev, 0xBA, &data, 1);	// Active Area LSB of Y2
		data = 0x05;
		ret |= ft8006p_reg_write(dev, 0xBB, &data, 1);	// Active Area MSB of Y2 (1406)
		break;
...
static int ft8006p_lpwg_control(struct device *dev, u8 mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct tci_info *info1 = &ts->tci.info[TCI_1];
	//struct ft8006p_data *d = to_ft8006p_data(dev);
...
//		ret = ft8006p_tci_control(dev, TCI_CTRL_CONFIG_TCI_1);
	...
	TOUCH_I("ft8006p_lpwg_control mode = %d\n", mode);
...
//	mfts_mode = touch_check_boot_mode(dev);
	if ((ts->boot_mode >= TOUCH_MINIOS_MFTS_FOLDER) && !ts->role.mfts_lpwg) {
	...
	if (atomic_read(&ts->state.fb) == FB_SUSPEND) {
		if ((ts->role.mfts_lpwg) && (d->state == TC_STATE_ACTIVE)) {
			ft8006p_lpwg_control(dev, LPWG_DOUBLE_TAP);
			ft8006p_swipe_control(dev);
	...
	if (d->state == TC_STATE_ACTIVE) {
		if (ts->lpwg.screen == 0) {
			if((ts->lpwg.mode == 0) && !ts->role.mfts_lpwg) {
				...
				next_state = TC_STATE_LPWG;
				TOUCH_I("%s\n","STATE_LPWG");
				ret = ft8006p_lpwg_control(dev, ts->lpwg.mode); // LCD SLP In or LCD Deep Sleep, 100ms -> Touch LPWG
				ret = ft8006p_swipe_control(dev);
			}
			else {
				next_state = TC_STATE_DEEP_SLEEP;
...
static int ft8006p_notify_charger(struct device *dev, ulong event, void *data)
{
...
static int ft8006p_notify_normal(struct device *dev, ulong event, void *data)
{
	struct touch_core_data *ts = to_touch_core(dev);
//	struct ft8006p_data *d = to_ft8006p_data(dev);
	int ret = 0;
	...
	case LCD_EVENT_TOUCH_RESET_END:
		...
		if ((ts->boot_mode >= TOUCH_MINIOS_AAT) && ts->role.mfts_lpwg) {
			touch_msleep(ts->caps.hw_reset_delay);
...
//		atomic_set(&d->watch.state.rtc_status, RTC_CLEAR);
	...
	case LCD_EVENT_LCD_MODE:
		...
		ft8006p_lcd_mode(dev, *(u32 *)data);
		ret = ft8006p_check_mode(dev);
		if (ret == 0)
	...
	case LCD_EVENT_READ_REG:
		...
		ft8006p_lcd_event_read_reg(dev);
		break;
#endif
	case NOTIFY_CONNECTION:
		TOUCH_I("NOTIFY_CONNECTION!\n");
		ret = ft8006p_usb_status(dev, *(u32 *)data);
//		if (ft8006p_asc_usable(dev))	/* ASC */
//			ft8006p_asc_toggle_delta_check(dev);
		break;
#if 0
	case NOTIFY_WIRELEES:
		TOUCH_I("NOTIFY_WIRELEES!\n");
		ret = ft8006p_wireless_status(dev, *(u32 *)data);
//		if (ft8006p_asc_usable(dev))	/* ASC */
//			ft8006p_asc_toggle_delta_check(dev);
		break;
	case NOTIFY_EARJACK:
		TOUCH_I("NOTIFY_EARJACK!\n");
		ret = ft8006p_earjack_status(dev, *(u32 *)data);
		break;
#endif
	case NOTIFY_IME_STATE:
		TOUCH_I("NOTIFY_IME_STATE!\n");
//		ret = ft8006p_reg_write(dev, REG_IME_STATE,
//			(u32*)data, sizeof(u32));
		break;
#if 0
	case NOTIFY_DEBUG_TOOL:
		ret = ft8006p_debug_tool(dev, *(u32 *)data);
...
//			ft8006p_asc_toggle_delta_check(dev);
	...
	case NOTIFY_DEBUG_OPTION:
		...
		ret = ft8006p_debug_option(dev, (u32 *)data);
		break;
	case NOTIFY_ONHAND_STATE:
		TOUCH_I("NOTIFY_ONHAND_STATE!\n");
//		if (ft8006p_asc_usable(dev)) {	/* ASC */
//			ft8006p_asc_toggle_delta_check(dev);
//			ft8006p_asc_write_onhand(dev, *(u32 *)data);
//		}
		break;
#endif
/*	case NOTIFY_QUICKCOVER_STATE:
		TOUCH_I("NOTIFY_QUICKCOVER_STATE!: %d\n", *(u8 *)data);
		ret = ft8006p_reg_write(dev, SPR_QUICKCOVER_STS, (u8 *)data, sizeof(u8));
		break; */
	default:
...
static int ft8006p_notify(struct device *dev, ulong event, void *data)
{
	...
	TOUCH_TRACE();
        ...
        if (boot_mode == TOUCH_CHARGER_MODE
                ...
                return ft8006p_notify_charger(dev,event,data); 
        }

        return ft8006p_notify_normal(dev,event,data); 
...
static int ft8006p_probe(struct device *dev)
{
	...
	d->swipe_debug_type = SWIPE_DEBUG_DISABLE;
	atomic_set(&ts->state.debug_option_mask, DEBUG_OPTION_2);
...
#if defined(CONFIG_LGE_TOUCH_MODULE_DETECT)
		if (ft8006p_panel_type == BOE_INCELL_FT8006P) {
...
/*
	mfts_mode = touch_check_boot_mode(dev);
	if ((mfts_mode >= TOUCH_MINIOS_MFTS_FOLDER) && !ts->role.mfts_lpwg) {
...
#if 1
	//if(tci_data_buf[0] == SWIPE) {
	if(tci == SWIPE) {
		TOUCH_I("SWIPE Up Check\n");
	} else if (!((tci_data_buf[0] == 0x01 && tci == TCI_1) || (tci_data_buf[0] == 0x02 && tci == TCI_2))
		|| tci_data_buf[1] == 0 || tci_data_buf[1] > MAX_TAP_COUNT) {
		TOUCH_I("tci data is invalid!!\n");
		return -1;
	}
#else
	if (!((tci_data_buf[0] == 0x01 && tci == TCI_1) || (tci_data_buf[0] == 0x02 && tci == TCI_2) || (tci_data_buf[0] == 0x04 && tci == SWIPE))
		|| tci_data_buf[1] == 0 || tci_data_buf[1] > MAX_TAP_COUNT) {
		TOUCH_I("tci data is invalid!!\n");
		return -1;
	}
...
int ft8006p_irq_report_swipe_fr(struct device *dev)
{
	...
	if (d->swipe_debug_type != SWIPE_DEBUG_ALWAYS && d->swipe_debug_type != SWIPE_DEBUG_BUFFER_ALWAYS) {
	...
	if (swipe_fr < SWIPE_FR_NUM) {
		TOUCH_I("Swipe-up Failure Reason Reported : [%s]\n", swipe_debug_str[swipe_fr]);
	}
	else {
		TOUCH_I("Swipe-up Failure Reason Reported : [%s]\n", swipe_debug_str[SWIPE_FR_NUM]);
	}
...
//#if defined(CONFIG_LGE_DISPLAY_RECOVERY_ESD) /* Doesn't use this Mecro in MTK */
		ret = -EGLOBALRESET;
//#endif
	}

        else if (int_status == 0x06) { // SWIPE UP
                ret = ft8006p_irq_lpwg(dev, SWIPE);
        }

        else if (int_status == 0x0A) { // SWIPE Fail Reason
...
static ssize_t show_swipe_debug(struct device *dev, char *buf)
{
	...
	ret = snprintf(buf + ret, PAGE_SIZE,
			...
			(d->swipe_debug_type < 4) ? swipe_debug_str[d->swipe_debug_type] : "Invalid");
...
static ssize_t store_swipe_debug(struct device *dev, const char *buf, size_t count)
{
	...
	TOUCH_I("Set SWipe Debug Type = %s\n", (d->swipe_debug_type == 1) ? swipe_debug_str[d->swipe_debug_type] : "Invalid");
...
static int __init touch_device_init(void)
{
	...
	ft8006p_panel_type = lcm_id;	

	switch (lcm_id) {
	case CPT_INCELL_FT8006P:
		TOUCH_I("%s, TXD-CPT-ft8006p found!\n", __func__);
		break;
	case BOE_INCELL_FT8006P:
		TOUCH_I("%s, LCE-BOE-ft8006p found!\n", __func__);
		break;
	default:
	...
	lcm_id = lge_get_maker_id();
	ft8006p_panel_type = lcm_id;	

	switch (lcm_id) {
	case BOE_INCELL_FT8006P:
		TOUCH_I("%s, TXD-ft8006p found!\n", __func__);
		break;
	case CPT_INCELL_FT8006P:
		TOUCH_I("%s, LCE-ft8006p found!\n", __func__);
		break;
	default:
		TOUCH_I("%s, ft8006p not found.\n", __func__);
		return 0;
