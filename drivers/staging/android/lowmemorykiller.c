...
hswap_kill:
		...
		lowmem_print(1, "%s '%s' (%d) (tgid %d), adj %hd,\n"
			"   reclaimable_cnt %d, top (%ld, %d), reason %s\n"
#endif
			"   to free %ldkB on behalf of '%s' (%d) because\n"
...
#ifdef CONFIG_HSWAP
			reclaimable_cnt, selected->signal->top_time,
			selected->signal->top_count, kill_reason_str[kill_reason],
#endif
