...
static void fg_adjust_vfloat_rescale_offset(struct fg_chip *fg)
{
	int rc = 0;
	...
	if (!is_charger_available(fg)) {
		pr_err("Charger not available yet!\n");
		return;
	}

	vfloat_set = fg->cbc_adjusted_vfloat;
	...
	if (vfloat_set != val.intval) {
		fg->cbc_adjusted_vfloat = val.intval;
		fg->cc_cv_threshold_mv = fg->cbc_adjusted_vfloat - 10;
		full_chg_offset = (fg->batt_max_voltage_uv/1000) - fg->cbc_adjusted_vfloat;
		full_chg_offset = full_chg_offset*2/10; //FG Raw SOC rescale fomular.
		rescale.criteria = rescale.dt_criteria - full_chg_offset;
		pr_info("rescale.criteria: %d, full_chg_offset = %d\n",
			rescale.criteria, full_chg_offset);
		pr_info("cc_cv_threshold_mv: %d, cbc_adjusted_vfloat = %d\n",
			fg->cc_cv_threshold_mv, fg->cbc_adjusted_vfloat);
		update_cc_cv_setpoint(fg);
	}
}
#endif
#endif
...
#ifdef CONFIG_LGE_PM_CCD
#ifndef CONFIG_LGE_PM_QNOVO_QNS
		if (msoc_raw > rescale.criteria){
			fg_adjust_vfloat_rescale_offset(fg);
		}
#endif
#endif
...
#ifdef CONFIG_LGE_PM_CCD
#ifndef CONFIG_LGE_PM_QNOVO_QNS
	rescale.dt_criteria = rescale.criteria;
#endif
#endif
