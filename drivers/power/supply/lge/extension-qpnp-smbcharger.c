...
static int get_usb_adc(struct smbchg_chip *chip)
{
    ...
    if (IS_ERR_OR_NULL(usbin_vadc_dev)) {
        usbin_vadc_dev =
              qpnp_get_vadc(chip->dev, "usb_pin");
        if (IS_ERR_OR_NULL(usbin_vadc_dev)) {
...
static int get_usb_adc_uvol(struct smbchg_chip *chip)
{
    struct qpnp_vadc_result results;
    int rc, usbin_vol;
...
#define QNOVO_PTRAIN_EN                 0x1549
	...
	if (presence_usb) { // On Wired charging
		...
		int usb_vnow = get_usb_adc(chip); // unit : mv
...
// Read ADC if possible
	rc |= qpnp_vadc_read(vadc_dev, P_MUX1_1_1, &result);
	if (rc < 0)
...
static bool extension_usb_set_current_max(/*@Nonnull*/struct smbchg_chip *chip, int current_max) {
	...
	if (fabproc && chgable) {
		smbchg_set_sdp_current(chip, FABCURR / 1000);
		if (icl != FABCURR)
...
// Pre-loading conditions
	...
	if (online && (chgtype != POWER_SUPPLY_TYPE_USB
				&& chgtype != POWER_SUPPLY_TYPE_USB_CDP
				&& chgtype != POWER_SUPPLY_TYPE_UNKNOWN))
		online = true;
	else
		online = false;
...
static bool extension_usb_port_get_online(/*@Nonnull*/ struct power_supply *psy) {
	...
	pr_debug("chgtype=%s, online=%d, fo=%d, pc=%d, usb=%d\n",
		log_psy_type(chgtype), online, fo, pc, usb);
...
#if 0
static void extension_usb_set_pd_active(/*@Nonnull*/ struct smbchg_chip *chip, int pd_active) {
	struct power_supply* veneer = power_supply_get_by_name("veneer");
...
int extension_usb_get_property(struct power_supply *psy,
	...
	case POWER_SUPPLY_PROP_VOLTAGE_NOW :
		val->intval = get_usb_adc_uvol(chip);
		return 0;
