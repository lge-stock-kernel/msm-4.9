...
static int adc_usbid_ohm(struct device* dev, int uvoltage) {
	...
	if (pullup_mvol == 0) {
		rc = of_property_read_s32(of_find_node_by_name(dev->of_node,
			"lge-extension-usb"), "lge,usbid-pullup-mvol", &pullup_mvol);
		if (rc < 0){
			pr_err("USB-ID: Error on reading property. \n");
			return rc;
		}
...
static int psy_low_pcb_get(struct smb_charger* chg) {
	struct device* dev = chg->dev;
	static struct qpnp_vadc_chip* lowpcb_vadc = NULL;
	struct qpnp_vadc_result results = {
		.physical = 0,
		.adc_code = 0,
	};
	static int low_pcb_channel = -1;
	int rc = 0;

	mutex_lock(&low_pcb_adc_mutex);
	if (IS_ERR_OR_NULL(lowpcb_vadc)) {
		lowpcb_vadc = qpnp_get_vadc(dev, "lowpcb");
	}
	if (IS_ERR_OR_NULL(lowpcb_vadc)) {
		pr_info("LOW-PCB: %s to get qpnp-vadc\n",
			IS_ERR_OR_NULL(lowpcb_vadc)? "Failed" : "Success");
		mutex_unlock(&low_pcb_adc_mutex);
		return -EINVAL;
	}
	if (low_pcb_channel < 0) {
		rc = of_property_read_s32(of_find_node_by_name(dev->of_node,
			"lge-extension-adc"), "lge,low-pcb-adc-channel", &low_pcb_channel);
		if (rc < 0){
			pr_err("USB-ID: Error on reading property. \n");
			return rc;
		}
		pr_info("LOW-PCB: Success to get low-pcb-adc-channel: %d\n",
			low_pcb_channel);
	}

	rc = qpnp_vadc_read(lowpcb_vadc, low_pcb_channel, &results);
	if (rc < 0) {
		pr_err("LOW-PCB: Error on getting LOW-PCB ADC(uvol)\n");
	}
	mutex_unlock(&low_pcb_adc_mutex);

	return !rc ? results.physical : 0;
}
#endif
...
static int moisture_mode_off(struct smb_charger* chg) {
	...
	// ~2. enable apsd
	vote(chg->apsd_disable_votable, MOISTURE_VOTER, false, 0);
...
int extension_usb_get_property(struct power_supply *psy,
	...
	case POWER_SUPPLY_PROP_LOW_PCB :
		val->intval = psy_low_pcb_get(chg);
		return 0;
#endif
...
int extension_usb_main_set_property(struct power_supply* psy,
	...
	case POWER_SUPPLY_PROP_CURRENT_MAX : {
		...
		if (fabproc && chgable) {
			...
			int rc = 0;
			u8 reg = 0;
			int fabcurr = get_client_vote_locked(charger->usb_icl_votable, VENEER_VOTER_IUSB);
			...
			rc = smblib_masked_write(charger, USBIN_ICL_OPTIONS_REG,
				USBIN_MODE_CHG_BIT | CFG_USB3P0_SEL_BIT | USB51_MODE_BIT,
				USBIN_MODE_CHG_BIT);
			if (rc < 0) {
				pr_err("Couldn't set ICL OPTIONS REG rc=%d\n", rc);
				break;
			}
