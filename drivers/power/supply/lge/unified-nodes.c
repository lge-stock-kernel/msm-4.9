...
#ifdef CONFIG_LGE_PM_LAOP_ATT_CRK_SIM_BASED_SWITCHING
static ssize_t charger_verbose_att_op_store(struct device* dev, struct device_attribute* attr, const char* buf, size_t size) {
	struct unified_nodes*   ref;
	bool*                   ori;
	int             new;
	...
	if (dev && dev->platform_data) {
		ref = (struct unified_nodes*)dev->platform_data;
		ori = &ref->charger_verbose_att_op;
...
static ssize_t charger_verbose_att_op_show(struct device* dev, struct device_attribute* attr, char* buf) {
	int ret = 0;
	if (dev && dev->platform_data) {
		ret = ((struct unified_nodes*)dev->platform_data)->charger_verbose_att_op;
	}
...
#ifdef CONFIG_LGE_PM_LAOP_ATT_CRK_SIM_BASED_SWITCHING
	__ATTR(charger_verbose_att_op,	0664, charger_verbose_att_op_show,	charger_verbose_att_op_store),
#endif
