...
/* v/i ADCs */
	...
	int battery_ocv;
	int input_vusb;
	//int input_iusb; //block for ADC stability
	int input_aicl;
/* Temperature */
	int temp_compensated;
	//int temp_thermister;  //block for ADC stability
	int temp_vts;
/* Impedance */
	int impedance_esr;
/* Misc */
...
/* v/i ADCs  */ LGE_QG_INITVAL, LGE_QG_INITVAL,
                LGE_QG_INITVAL, LGE_QG_INITVAL, LGE_QG_INITVAL,
/* Temp      */ LGE_QG_INITVAL, -1000,
...
static int calculate_battery_temperature(/* @Nonnull */ struct qpnp_qg *qg)
{
	...
	if(check_battery_temp_update_time()){
		if (qg_get_battery_temp(qg, &battemp_cell)){
...
/* v/i ADCs  */ "iBAT:%d, vBAT:%d, vOCV:%d, vUSB:%d, AiCL:%d\n"
				"PMINFO: [CSV] "
/* Temp      */ "tSYS:%d, tVTS:%d, "
/* Impedance */ "rESR:%d, "
/* Misc      */ //"CYCLE:%d, "
...
/* Input     */ qginfo.input_vusb, qginfo.input_aicl,
/* Temp      */ qginfo.temp_compensated, qginfo.temp_vts,
...
static void
qginfo_snapshot_inputnow(int* vusb, int* aicl)
{
...
/* Battery */
	...
	qginfo.battery_ocv = !power_supply_get_property(qg->qg_psy,
		...
		? val.intval / 1000 : LGE_QG_INITVAL;
/* Input */
	qginfo_snapshot_inputnow(&qginfo.input_vusb, &qginfo.input_aicl);
/* Temperature */
