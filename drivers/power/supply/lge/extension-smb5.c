...
#define CCD_VFLOAT_LIMIT_THRESHOLD 4300
	...
	if (mvalue != VOTE_TOTALLY_BLOCKED) {
		if (mvalue == VOTE_TOTALLY_RELEASED) {
			...
			*/
			...
			uv_now = val.intval;
			pr_debug("uv_now : %d\n", uv_now);
			if ((uv_float < CCD_VFLOAT_LIMIT_THRESHOLD * 1000) && (uv_now > uv_float
...
#ifdef CONFIG_LGE_PM_USBID_RE_READ
#define ADC_READ_COUNT 3
#define GOOD_ADC_RANGE_DIFF	100000
static bool psy_usbid_update(struct device* dev) {
	int rc = 0;
	int adc_read_bucket[ADC_READ_COUNT];
	int first_read_adc = 0;
	int i =0;
	int usbid_uvoltage = 0;
...
// Update all
	...
	if (vadc_dev) {
		if (unified_bootmode_adc_retry()){
			first_read_adc = adc_usbid_uvoltage(dev, vadc_dev);
			pr_err("USB-ID: first_read_adc = %d uvol \n", first_read_adc);
			for (i=1; i<ADC_READ_COUNT; i++){
				msleep(100);
				adc_read_bucket[i] = adc_usbid_uvoltage(dev, vadc_dev);
				pr_err("USB-ID: read adc try %d = %d uvol \n", i, adc_read_bucket[i]);
				if ((usbid_open_min_mvol > 0) &&
					( first_read_adc + adc_read_bucket[i] > usbid_open_min_mvol*2)){
					pr_err("USB-ID: open cable = %d uvol \n", first_read_adc);
					usbid_uvoltage = first_read_adc;
					break;
				}
				if(abs(first_read_adc - adc_read_bucket[i]) < GOOD_ADC_RANGE_DIFF){
					pr_err("USB-ID: GOOD_ADC_RANGE_DIFF = %d uvol \n", first_read_adc);
					usbid_uvoltage = first_read_adc;
				} else {
					pr_err("USB-ID: NOT GOOD_ADC_RANGE_DIFF try %d = %d uvol \n", i, adc_read_bucket[i]);
					usbid_uvoltage = adc_read_bucket[i];
				}
			}
		}else
			usbid_uvoltage = adc_usbid_uvoltage(dev, vadc_dev);
		cache_usbid_type     = adc_usbid_type(dev, usbid_uvoltage/1000);
		pr_err("USB-ID: Updated to %d uvol => %s\n",
			usbid_uvoltage, adc_usbid_name(cache_usbid_type));
	}
	else
	...
	if (usbid_uvoltage > 0)
		cache_usbid_uvoltage = usbid_uvoltage;
	else
		pr_err("USB-ID: Error on reading USBID ADC(mvol)\n");

// Check validation of result
	return usbid_uvoltage > 0;
}
#else
static bool psy_usbid_update(struct device* dev) {
	int rc = 0;
...
static int moisture_mode_on(struct smb_charger* chg) {
...
#endif
	u8 reg_val = 0;
	pr_err("moisture mode off chg->moisture_ux = %d\n", chg->moisture_ux);
	if (moisture_charging == 0) {
		...
		// 4. if not moisture_ux Set input current 1000mA
		if(!chg->moisture_ux){
...
#ifndef CONFIG_LGE_USB_TYPE_C
	// ~5. TYPEC_PR_DUAL for power role
	val.intval = POWER_SUPPLY_TYPEC_PR_DUAL;
	power_supply_set_property(chg->usb_psy, POWER_SUPPLY_PROP_TYPEC_POWER_ROLE, &val);
#endif

	// ~3. if not moisture_ux enable HVDCP
	if(!chg->moisture_ux) {
		reg_val = (HVDCP_AUTH_ALG_EN_CFG_BIT | HVDCP_EN_BIT);
		rc = smblib_masked_write(chg, USBIN_OPTIONS_1_CFG_REG,
				(HVDCP_AUTH_ALG_EN_CFG_BIT | HVDCP_EN_BIT |
				 HVDCP_AUTONOMOUS_MODE_EN_CFG_BIT),
				reg_val);
