...
#define CLKS_XO 1
#define CLKS_PMIC 2
static unsigned int clk_source = CLKS_PMIC;
#endif
        ...
        const struct i2c_device_id *id)
{
    ...
    pr_info(PN547_DRV_NAME ":IRQ : %d  VEN : %d  FIRM : %d\n",
...
#ifdef CONFIG_LGE_NFC_USE_PMIC
    ret = gpio_request(pn547_dev->clkreq_gpio, "nfc_clkreq");
    if (ret) {
        pr_info(PN547_DRV_NAME ":pn547_probe() : nfc_clkreq request failed!\n");
        goto err_clkreq;
    }
    pr_info("%s :  pn547_dev->clkreq_gpio %d\n", __func__, pn547_dev->clkreq_gpio);
#endif
...
#ifdef CONFIG_LGE_NFC_USE_PMIC
    ret = gpio_direction_input(pn547_dev->clkreq_gpio);
