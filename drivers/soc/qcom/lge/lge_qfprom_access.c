...
#define MAGIC_NUM1 0x3b7251ca
#define MAGIC_NUM2 0x2a126f29
...
static DEVICE_ATTR(mba, S_IWUSR | S_IRUGO, qfprom_read_version_show, NULL);
static DEVICE_ATTR(modem, S_IWUSR | S_IRUGO, qfprom_read_version_show, NULL);
...
static u32 qfprom_verification_blow_data(void)
{
  ...
  for (i = 0 ; i < secdat.list_hdr.fuse_count; i++) {
    ...
    switch(qfuse->addr)
    {
      ...
      case QFPROM_ANTIROLLBACK1:
      case QFPROM_ANTIROLLBACK2:
      case QFPROM_ANTIROLLBACK3:
        break;
...
static u32 qfprom_read(u32 fuse_addr)
{
  ...
  if(fuse_addr ==  QFPROM_SEC_HW_KEY){
	...
	if (value_addr == 0)
	{
	    printk(KERN_ERR "[QFUSE]%s value_addr is 0\n", __func__);
	    return 0;
	 }
...
static u32 qfprom_secdat_read(void)
{
  ...
  if(secdat.hdr.magic1 != MAGIC_NUM1 && secdat.hdr.magic2 != MAGIC_NUM2){
    printk(KERN_ERR "[QFUSE]%s : hdr magic error\n", __func__);
    ret = RET_ERR;
    goto err_mem;
  }
