...
#ifdef CONFIG_MIGRATE_HIGHORDER
#ifndef CONFIG_64BIT
#ifndef CONFIG_LOW_TIER_DEVICE /* To use normal zone for highorder in low tier device */
		if (is_highmem(zone))
#else
		if (!is_highmem(zone))
#endif
...
#ifdef CONFIG_HSWAP
		if (!is_highmem(zone)) {
			zone->watermark[WMARK_HIGH] = low
				+ (min_wmark_pages(zone) * (wmark_tune_level + 1));
